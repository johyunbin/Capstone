...
cat <<EOF
{"hookSpecificOutput": {"hookEventName": "SessionStart", "additionalContext": "캡스톤 프로젝트 | branch: ${BRANCH} | git pull: ${PULL_STATUS} | 미커밋: ${UNCOMMITTED}건 | 분석문서: ${RESEARCH_COUNT}md | 원논문: ${PAPERS_COUNT}pdf | 템플릿: ${HAS_TEMPLATES} | 최근커밋: ${LAST_COMMIT} | RC: ${RC_STATUS}"}}
EOF
