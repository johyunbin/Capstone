...
## 동적 state + 인계 (5/20 18:08 갱신 — Phase 2-B 압축 48→14p + figure 한글 폰트 fix + md2pdf 2 버전 분기)

> CLAUDE.md = 라우팅 + 안정 룰. 동적 state·진행 수치는 handoff·v13 정본에 있다 — 새 세션은 아래 anchor 하나로 0% loss 인계.

- **★ 새 세션 진입 anchor (0% loss)**: `@_internal/handoff/active/handoff_20260520_180804_Phase2B압축_figurefix_md2pdf2버전.md` (Phase 2-B 본문 압축 _162500 48p → **신본 _171521 compact 14p** (71% 축소·정본 17/17·교수님 12/12 충족) + readable 23p 내부 검토 + figure 5-1·5-2 한글 폰트 fix (analyze_latency.py matplotlib rcParams) + md2pdf 2 버전 분기 (md2pdf.py compact 제출용 + md2pdf_readable.py 내부 검토) + 빈 여백 fix (subsection-keep 해제 → 17p → 14p). 학교 양식 14~22p 최저 충족. 정본 수치 carry: 3-7× · 5.67× · 94.9% · 7/12 · 148/156 · 180/180 · 13/16 · 89.1% · 1,508 · 2,880 · 86.9% · plan_signature = Node Type 1-tuple · q9 sel=0.1 honest exception · v2 PPTX ship-ready (raster image, 21장). 다음 = 5/22 미팅·5/25 팀원 검토·5/26 11:59 PM PPTX Learnus 마감·5/27+5/29 양일 발표·5/28 12:00 포스터 PDF·6/5 금 전시회·6/10 박광현 마지막 세미나·6/11 보고서·상호평가 결과)
...
- **★ 6/11 최종 보고서 신본 (정본 = compact 14p)**: `@submission/_drafts/속도는벡터_6_11_최종보고서_20260520_171521.md` (+ `.pdf` 14p · `.docx`) — 정본 17/17 + 교수님 12/12 충족. readable 23p 는 `_171521_readable.pdf` (내부 검토). 직전 _162500 (48p)·_144446·_124200 본은 carry.
- **★ md2pdf 2 버전 (신규 분기)**: `@_internal/scripts/md2pdf.py` (compact 제출용 default — margin 11/12mm·font 9.8pt·H2 자연 흐름·subsection-keep auto) + `@_internal/scripts/md2pdf_readable.py` (readable 내부 검토 — margin 18mm·font 10.5pt·H2 break·subsection-keep avoid·git HEAD 원본)
