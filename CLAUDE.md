...
> **실행 단계** — 연구 방향 전환(4/3), 설계안 v3 확정, 환경 수령 대기 중

- 연구 방향: **Skew-Aware Sampling** — skewed 거리 분포에서 카디널리티 추정 정확도 개선
  - 트랙 A (Distribution-Aware): 분포를 알 때 → 층화 샘플링(stratified sampling)
  - 트랙 B (Distribution-Agnostic): 분포를 모를 때 → KDE-pilot 기반 자동 층화
- 설계안 확정: `plans/연구_설계안_20260403_162818.md` (v3, Cascade 방향 폐기)
- **연구제안서 + 수행계획서 제출 완료** (4/2 LearnUs 제출)
- 논문 분석: **82/82편 완료 (100%)** + 시리즈 12편 + 원논문 69편
- 지침 시스템: 9대 지침 첫 실행 완료 (00점검/04문서/05주간/06미팅/08설계)
- 다음 할 일: LHS·층화 샘플링 이론 정리 → Exqutor 코드/데이터 수령 후 실험
- 환경: 연구실 서버 제공 확정, 석사분 Exqutor 코드+데이터 다음 주 전달 예정
...
| 마감 | 제출물 | 상태 |
|------|--------|------|
| ~3/29 | 연구지도 확인서 1~4회차 | ✅ (4회차 3/29 제출) |
| 3/26 | 1차 자문내역서 + 자문컨택내역 (런어스 제출) | ✅ |
| 3/28 | 교수님 미팅 (방향 확정) | ✅ |
| 4/1 | 세미나 (초청 강연, 전해곤 교수님) | ✅ |
| 4/2 | 연구제안서 + 수행계획서 제출 | ✅ (박세은 LearnUs 제출) |
| 4/3 | 교수님 미팅 — 연구 방향 전환 (샘플링) | ✅ |
| 4월 중 | 실험 설계 확정 | ✅ (설계안 v3 4/3 확정) |
| 4월 둘째 주 | Exqutor 코드·데이터 수령 + 환경 세팅 | ⬜ |
| **4/28** | **중간발표 + 중간보고서 제출** | ⬜ |
...
→ `plans/연구_설계안_20260403_162818.md` 참조 (RQ3개: Motivation/Aware/Agnostic, 지표4, 3단계 실험)
