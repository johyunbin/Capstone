...
DATASET_ALIAS = {
    ...
    # concat 측정 트랙 (5/16) — build_concat_cells.py 가 만든 query_pool_{ALIAS}_sf{N}.parquet 와 lock-step.
    # concat 공간 query pool 은 concat 벡터 자체로 만들어졌으므로 별도 alias 필수 (DEEP query 재사용 X).
    "DEEP+SIFT concat": "DEEP_SIFT_CONCAT",   # 224d (DEEP 96 + SIFT 128)
    "DEEP+WIKI concat": "DEEP_WIKI_CONCAT",   # 864d (DEEP 96 + WIKI 768)
    "DEEP+YFCC concat": "DEEP_YFCC_CONCAT",   # 288d (DEEP 96 + YFCC 192)
}
...
def build_cell_specs() -> list[CellSpec]:
    ...
    # === concat 측정 트랙 (5/16) — multi-vector concat 7 cell ===
    # build_concat_cells.py 가 만든 NPY/parquet artifact 와 lock-step.
    #   - table  : NPY fast-path 가 cache/rq1/{table}_vectors.npy + _strata.npy 를 읽음
    #   - dataset: DATASET_ALIAS 로 query_pool_{ALIAS}_sf{N}.parquet 매핑 (concat 공간 query pool)
    #   - vec_dim: concat 차원 (DEEP 96 + B). embed_col 은 NPY fast-path 에선 미사용이라 더미 OK.
    #   - mode_pool: B1 + CaseB 만 (CaseA 단독 대체 폐기 — 사용자 정책)
    # query SQL / threshold 는 A8 패턴(TPC-H 8 query) 그대로 — concat cell 도 partsupp 기반.
    for combo_ds, combo_tbl, dim_b in [
        ("DEEP+SIFT", "deep_sift", 128),   # 224d
        ("DEEP+WIKI", "deep_wiki", 768),   # 864d
        ("DEEP+YFCC", "deep_yfcc", 192),   # 288d
    ]:
        # DEEP+SIFT 는 sf 1/10/100, DEEP+WIKI / DEEP+YFCC 는 sf 1/10 (소스 NPY 가용 범위)
        sf_list = [1, 10, 100] if combo_tbl == "deep_sift" else [1, 10]
        # A9 = DEEP+SIFT, A10 = DEEP+WIKI, A11 = DEEP+YFCC
        sub_no = {"deep_sift": 9, "deep_wiki": 10, "deep_yfcc": 11}[combo_tbl]
        for sf in sf_list:
            cells.append(CellSpec(
                sub=f"A{sub_no}-{combo_ds}-concat-sf{sf}",
                fig=f"concat 트랙 multi {96 + dim_b}d",
                dataset=f"{combo_ds} concat",          # → DATASET_ALIAS → query_pool_{ALIAS}_sf{N}
                sf=sf,
                table=f"partsupp_{combo_tbl}_concat_{sf}",
                embed_col="ps_embedding",              # NPY fast-path 미사용 더미
                vec_dim=96 + dim_b,                     # concat 차원
                queries=TPC_H_QUERIES,
                selectivities=[PAPER_SEL_DEFAULT],      # A8 동일 — sel sweep 은 별도 selectivities 인자로
                threshold_map={q: TPC_H_THRESHOLD for q in TPC_H_QUERIES},
                mode_pool=["B1", "CaseB"],
            ))
...
def measure_b1_paper(cell: CellSpec, n_queries: int = 1000,
                     ...
                     sel_override: Optional[float] = None) -> dict:
    ...
    for trial_idx in range(trials):
        ...
        for q_idx in range(n_queries):
            ...
            # sel_override 우선 (RQ3 sel sweep), 없으면 기존 동작 (cell.selectivities[0])
            sel = sel_override if sel_override is not None else (cell.selectivities[0] if cell.selectivities[0] is not None else PAPER_SEL_DEFAULT)
            ...
            # K10 B1 fix (5/17): all_vecs 전달 → strata 캐시 우회, 전체 벡터 직접
            # random sample. B1 (대조군) 은 strata 수 K 와 무관해야 함.
...
def measure_case_b(cell: CellSpec, method_name: str, n_queries: int = 1000,
    ...
    for trial_idx in range(trials):
        ...
        for q_idx in range(n_queries):
            ...
            # 1단계 fix (5/17): all_vecs 전달 → 2단계 strata 캐시 우회, 전체 벡터 직접
            #   (B1 mode L469 와 동일 — CaseB est_b1 도 paper-faithful 1단계 Bernoulli 로 통일)
            est_b1 = mc.bernoulli_estimate(samples_b1, sizes_b1, qvec, D, rng,
                                           budget=state.size, all_vecs=all_vecs)
...
def measure_3way(cell: CellSpec, method_name: str, n_queries: int = 1000,
                 trials: int = TRIALS, output_dir: Optional[Path] = None,
                 sel_override: Optional[float] = None) -> dict:
    """3-way matched 측정 — 한 측정에서 B1·CaseA·CaseB 를 함께 산출 (5/17).

    같은 cell·sel·K·method·trial·query 조건에서 세 mode 를 동시에 측정한다.
    각 mode 는 자기 AdaptiveState (Eq 1-6) 로 sample budget 을 독립 적응:
        B1    : est = bernoulli_estimate(...)              # paper baseline (1단계)
        CaseA : est = stratified_estimate(method strata)   # 완전 대체
        CaseB : est = (est_b1 + est_method) / 2            # 결합/증강
    matched 3-way — REPORT v13 / narrative v8 의 B1/CaseA/CaseB 대조 입력.
    """
    if not SERVER:
        raise RuntimeError("server only")

    print(f"[{mc.kst()}] 3way paper exact: cell={cell.sub} method={method_name}")
    alias = DATASET_ALIAS.get(cell.dataset, cell.dataset)
    ds = {
        "name": cell.dataset, "table": cell.table,
        "embed_col": cell.embed_col, "vec_dim": cell.vec_dim,
    ...
    # per-query 기록: estimates(카디널리티 추정값) + q_errors. 추후 비교 metric
    # (Q-error만 / 카디널리티 추정까지) 결정을 위해 둘 다 남긴다. true_cards 는
    # query 결정적이라 measurement 당 1회(trial 0) 기록.
    tr_b1, tr_a, tr_b = [], [], []
    true_cards_rec = []
    sel = None
    for trial_idx in range(trials):
        rng = np.random.default_rng(trial_idx * 13 + 7)
        st_b1, st_a, st_b = AdaptiveState(), AdaptiveState(), AdaptiveState()
        qe_b1, qe_a, qe_b = [], [], []
        est_b1, est_a, est_b = [], [], []
        ...
        for q_idx in range(n_queries):
            ...
            if len(qs_match) > 0:
                ...
                true_card = total_rows * sel
            if trial_idx == 0:
                true_cards_rec.append(round(float(true_card), 4))

            # B1 — paper Bernoulli (1단계 all_vecs), 자기 AdaptiveState
            e_b1 = mc.bernoulli_estimate(samples_b1, sizes_b1, qvec, D, rng,
                                         budget=st_b1.size, all_vecs=all_vecs)
            qq = q_error(e_b1, true_card)
            est_b1.append(round(float(e_b1), 4)); qe_b1.append(qq)
            st_b1.update(qq, st_b1.size / total_rows)

            # CaseA — method stratified 단독, 자기 AdaptiveState
            alloc_a = mc.equal_alloc(n_strata=mc.N_STRATA, budget=st_a.size)
            e_a = mc.stratified_estimate(samples_method, sizes_method, alloc_a, qvec, D, rng)
            qq = q_error(e_a, true_card)
            est_a.append(round(float(e_a), 4)); qe_a.append(qq)
            st_a.update(qq, st_a.size / total_rows)

            # CaseB — (est_b1 + est_method) / 2 ensemble, 자기 AdaptiveState
            e_b1_b = mc.bernoulli_estimate(samples_b1, sizes_b1, qvec, D, rng,
                                           budget=st_b.size, all_vecs=all_vecs)
            alloc_b = mc.equal_alloc(n_strata=mc.N_STRATA, budget=st_b.size)
            e_m_b = mc.stratified_estimate(samples_method, sizes_method, alloc_b, qvec, D, rng)
            e_b = (e_b1_b + e_m_b) / 2.0
            qq = q_error(e_b, true_card)
            est_b.append(round(float(e_b), 4)); qe_b.append(qq)
            st_b.update(qq, st_b.size / total_rows)

        for qe, est, st, tr in [(qe_b1, est_b1, st_b1, tr_b1),
                                (qe_a, est_a, st_a, tr_a),
                                (qe_b, est_b, st_b, tr_b)]:
            finite = [v for v in qe if np.isfinite(v)]
            avg_qe = float(np.mean(finite)) if finite else float("inf")
            tr.append({
                "trial": trial_idx, "avg_q_error_finite": avg_qe,
                "n_finite": len(finite), "n_inf": len(qe) - len(finite),
                "final_size": st.size, "final_eta": st.eta,
                "estimates": est,
                "q_errors": [round(float(v), 6) if np.isfinite(v) else None for v in qe],
            })
        print(f"[{mc.kst()}]   trial {trial_idx+1}/{trials} "
              f"B1={tr_b1[-1]['avg_q_error_finite']:.3f} "
              f"CaseA={tr_a[-1]['avg_q_error_finite']:.3f} "
              f"CaseB={tr_b[-1]['avg_q_error_finite']:.3f}")

    def _summary(tr):
        avgs = [r["avg_q_error_finite"] for r in tr]
        return {
            "avg_q_error_trimmed": trimmed_mean(avgs, TRIM),
            "final_size_mean": float(np.mean([r["final_size"] for r in tr])),
            "final_size_std": float(np.std([r["final_size"] for r in tr])),
            "trial_results": tr,
        }

    result = {
        "cell": cell.sub, "fig": cell.fig, "dataset": cell.dataset, "sf": cell.sf,
        "mode": "3way", "method": method_name, "selectivity": sel,
        "ensemble_strategy": "simple_average",
        "n_queries": n_queries, "trials": trials,
        "fit_time_sec": fit_time_sec, "cache_time_sec": cache_time_sec,
        "true_cards": true_cards_rec,
        "B1": _summary(tr_b1),
        "CaseA": _summary(tr_a),
        "CaseB": _summary(tr_b),
        "kst": mc.kst(),
    ...
    if output_dir:
        output_dir.mkdir(parents=True, exist_ok=True)
        out = output_dir / f"{cell.sub}_3way_{method_name}.json"
        out.write_text(json.dumps(result, indent=2))
        print(f"[{mc.kst()}] saved {out}")
    return result
...
def main():
    ...
    parser.add_argument("--mode", choices=["B1", "CaseA", "CaseB", "ECQO", "3way"],
                        help="Measurement mode (RQ3 only). 3way = B1+CaseA+CaseB matched 동시 측정")
    ...
    parser.add_argument("--sel", type=float, default=None,
                        help="selectivity override (없으면 cell.selectivities[0]). RQ3 sel sweep 용")
    ...
    for cell in cells:
        if args.mode == "ECQO":
            ...
            if not args.method:
                print(f"[ERROR] --method required for 3way")
                return
            measure_3way(cell, args.method, args.n_queries, args.trials, args.output,
                         sel_override=args.sel)
