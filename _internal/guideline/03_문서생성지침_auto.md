...
- `_internal/scripts/md2pdf.py` 스크립트를 사용한 PDF 생성
...
- [ ] `_internal/scripts/md2pdf.py` 존재 확인
...
[ -f _internal/scripts/md2pdf.py ] && echo "md2pdf.py OK" || echo "md2pdf.py MISSING"
```
...
reference/analysis/와 reference/summaries/에서 md는 있지만 대응 pdf가 없는 파일을 찾는다.

- [ ] reference/analysis/ 내 md→pdf 누락 목록 생성
- [ ] reference/summaries/ 내 md→pdf 누락 목록 생성
...
for d in reference/analysis reference/summaries plans; do
  [ ! -d "$d" ] && continue
  for f in "$d"/*.md; do
    [ ! -f "$f" ] && continue
    pdf="${f%.md}.pdf"
...
# 단일 파일 테스트
python3 _internal/scripts/md2pdf.py "첫번째_누락파일.md"
...
for d in reference/analysis reference/summaries plans; do
  ...
  for f in "$d"/*.md; do
    ...
    pdf="${f%.md}.pdf"
    if [ ! -f "$pdf" ]; then
      if python3 _internal/scripts/md2pdf.py "$f"; then
        SUCCESS=$((SUCCESS + 1))
      else
        FAIL=$((FAIL + 1))
        echo "FAIL: $f"
...
ok, bad, bad_list = 0, 0, []
for d in ['reference/analysis', 'reference/summaries', 'plans']:
