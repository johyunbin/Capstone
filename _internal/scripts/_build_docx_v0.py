...
fill_cell(t2.rows[1].cells[1], "이채림 연구원", align=WD_ALIGN_PARAGRAPH.CENTER, size=11)
...
add_mixed_p(doc, [
    ("이러한 쿼리를 효율적으로 실행하려면 옵티마이저가 각 연산의 결과 행 수를 정확하게 추정해야 한다. 카디널리티 추정값은 인덱스 사용 여부, 조인 알고리즘과 순서 등 실행 계획의 거의 모든 결정에 영향을 미치기 때문이다. 그러나 pgvector·VBASE·DuckDB 등 주요 시스템은 카디널리티를 각각 33.3 %, 50 %, 100 % 의 고정 비율로 추정하기 때문에 0.001 % 에서 100 % 까지 변동하는 실제 카디널리티와 큰 괴리가 생기며, 이로 인한 잘못된 plan 선택은 실행 시간을 최대 4 orders of magnitude (약 1 만 배) 까지 증가시킨다. Exqutor 는 이 한계를 인덱스의 유무에 따라 두 경로로 보완한다. 인덱스가 있을 때는 ECQO (Exact Cardinality Query Optimization) 로 HNSW 인덱스에 range query 를 직접 한 번 실행하여 정확한 카디널리티를 얻고, 인덱스가 없을 때는 Adaptive Sampling 으로 동적 샘플링을 통해 카디널리티를 근사한다. 그러나 Exqutor 의 ", {}),
...
add_figure(doc, f"{FIG_DIR}/rq2_aware/figure_7_selectivity_gradient.png", width_inches=5.4,
           caption="그림 1. 세 데이터셋의 selectivity 별 KM20 vs BERN 개선율 (5-seed 95 % CI)")
...
add_figure(doc, f"{FIG_DIR}/rq2_aware/figure_9_two_level_decomposition.png", width_inches=5.4,
           caption="그림 2. Two-Level Decomposition — Level 1 (표본 안정화) 과 Level 2 (공간 인식) 의 분리")
...
add_figure(doc, f"{FIG_DIR}/rq2_aware/figure_10_cluster_skew.png", width_inches=5.4,
           caption="그림 3. DEEP 1M 과 SIFT 1.5M 의 K = 20 cluster 크기 분포 비교")
...
add_mixed_p(doc, [
    ...
    (" 패러다임은 데이터를 적재할 때 한 번만 계산해 두는 경량의 사전 처리에 해당한다. 여기에는 LSH 랜덤 하이퍼플레인 (A), Random Projection (C, Johnson-Lindenstrauss lemma 기반), Hilbert Curve (E, 공간 데이터베이스의 space-filling 기법을 벡터 카디널리티 추정에 처음 적용), Mini-batch K-means (F, 1 ~ 5 % 데이터만으로 근사 학습) 의 네 가지가 포함된다. ", {}),
    ("Online Query-Adaptive", {"bold": True}),
    (" 패러다임은 쿼리 시점에 pilot sample 로 동적으로 적응하는 방식으로, Distance-Shell (G, 쿼리 벡터 중심의 동심원) 과 KDE-pilot (B, Neyman 의 최적 배분) 의 두 가지를 포함한다. ", {}),
    ("Weight-based", {"bold": True}),
    (" 패러다임은 파티션 없이 가중치만으로 분산을 줄이는 방식으로, Importance Sampling (H) 한 가지를 포함한다. 일곱 가지 방법의 설계는 모두 확정된 상태이며, 실제 측정은 5 월의 비교 실험 단계에서 수행한다.", {})
])
...
add_p(doc, "표 3. 캡스톤 2026-1 학기 전체 일정",
      bold=False, size=10, align=WD_ALIGN_PARAGRAPH.CENTER, space_after=6)
...
out = f"/Users/hyunbin/Capstone/submission/_drafts/속도는벡터_중간보고서_{ts}.docx"
doc.save(out)
print(f"✅ saved: {out}")
