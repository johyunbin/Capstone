...
add_figure(doc, f"{FIG_DIR}/rq1_motivation/figure_1_phase4_scatter.png", width_inches=5.8,
           caption="그림 3.  SYSTEM ↔ BERNOULLI paired Q-error 의 4 selectivity panel (s = 0.05 / 0.10 / 0.30 / 0.50)")
...
add_figure(doc, f"{FIG_DIR}/rq1_motivation/figure_2_phase6_box.png", width_inches=5.6,
           caption="그림 4.  Phase 6 Step 4 — BERNOULLI ↔ STRATIFIED (KM20) 100 query × 6 selectivity boxplot (DEEP 1M, native vector.c 측정)")
...
add_mixed_p(doc, [
    ("실험 측면에서는 본 보고서에 보고된 모든 결과 — 표 1 의 SYSTEM ↔ BERNOULLI 비교, 표 2 의 5-seed 신뢰구간, 그림 1 ~ 8 의 vector.c 패치 시각화, RQ1 negative result heatmap, scatter / boxplot 직교 측정, 외적 타당성 cross-dataset bar, selectivity gradient, Two-Level decomposition, 그리고 cluster skew 비교 — 가 모두 확보된 상태이다. 모든 측정 데이터와 빌드 산출물은 재현 가능하도록 보존하였으며, 원본 코드와 수정본을 함께 유지하여 추후의 검증과 비교를 용이하게 하였다. RQ3 의 측정 결과는 5 월의 비교 실험을 마친 뒤 최종보고서에서 보고한다.", {})
])
