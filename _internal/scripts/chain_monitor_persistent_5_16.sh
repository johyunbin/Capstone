...
expected_count() {
    case "$1" in
        v6_caseB) echo 120 ;;       # B1 9 + 8 cell × 16 method ≈ 120
        v7_extras) echo 48 ;;       # 3 cell × 16 method
        v8_full) echo 600 ;;        # 12 cell × 50 method
        v9_sel_sweep) echo 680 ;;   # 16 cell × 2 sel × 17 method + K granularity 136
        v10) echo 0 ;;              # 미확정
        *) echo 0 ;;
    esac
}
...
while true; do
    ...
    REMOTE_OUT=$(ssh -o ConnectTimeout=10 -o ServerAliveInterval=5 "$SERVER" '
        ...
        for stage in v6_caseB v7_extras v8_full v9_sel_sweep v10; do
        ...
        for script in /tmp/v6_v7_chain.sh /tmp/v7_v8_chain.sh /tmp/v8_v9_chain.sh /tmp/v9_v10_chain.sh; do
            if [ -f "$script" ]; then
