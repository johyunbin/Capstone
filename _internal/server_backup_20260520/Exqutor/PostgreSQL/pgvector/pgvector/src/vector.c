...
static double estimate_cardinality_with_stratified_sampling(double total_rows, int n_strata);
static void load_stratum_sizes_cache(int n_strata);
...
/* 엔진 적용 검증 실험 — 카디널리티 주입 GUC */
static double vector_injected_card = -1.0;
static bool vector_disable_estimation = false;
...
/* Phase 6 Step 4: Stratified sampling GUC + cache */
#define VECTOR_MAX_STRATA 64
static char *sampling_method_str = NULL;
static char *stratified_table_column = NULL;
static int stratified_n_strata = 20;
static double stratum_sizes_cache[VECTOR_MAX_STRATA];
static bool stratum_sizes_cached = false;
static char stratum_sizes_cached_table[256] = "";
...
_PG_init(void)
{
    ...
    /* Phase 6 Step 4: stratified sampling GUC */
    DefineCustomStringVariable(
        "vector.sampling_method",
        "Sampling method (bernoulli or stratified).",
        NULL,
        &sampling_method_str,
        "bernoulli",
        PGC_USERSET,
    ...
    DefineCustomStringVariable(
        "vector.stratum_column",
        "Column name for stratum_id (stratified mode).",
        NULL,
        &stratified_table_column,
        "stratum_id",
        PGC_USERSET,
        0,
        NULL, NULL, NULL
    ...
    DefineCustomIntVariable(
        "vector.stratum_n",
        "Number of strata (stratified mode).",
        NULL,
        &stratified_n_strata,
        20,
        1, VECTOR_MAX_STRATA,
    ...
    DefineCustomRealVariable(
        "vector.injected_card",
        "Injected base-relation cardinality for the vector range predicate.",
        "When >= 0, used instead of sampling-based estimation.",
        &vector_injected_card,
        -1.0,
        -1.0, DBL_MAX,
        PGC_USERSET,
        0,
    ...
    DefineCustomBoolVariable(
        "vector.disable_estimation",
        "Disable Exqutor vector cardinality estimation (use planner default).",
        NULL,
        &vector_disable_estimation,
        false,
        PGC_USERSET,
        0,
        NULL, NULL, NULL
...
pgvector_set_baserel_rows_estimate_hook(PlannerInfo *root, RelOptInfo *rel)
{
	...
	{
		if (is_first_execution && ordering_needed && !is_sampling_active && !vector_disable_estimation)
		{
	...
	else if (IsA(node, SeqScan))
	{
		...
		if (strcmp(tableName, vector_table_name) == 0)
		{
			if (is_vector_range_query)
			{
				...
				if (vector_injected_card >= 0)
				{
					estimated_sample_rows = vector_injected_card;
				}
				else if (sampling_method_str != NULL && strcmp(sampling_method_str, "stratified") == 0)
				{
					estimated_sample_rows = estimate_cardinality_with_stratified_sampling(total_rows, stratified_n_strata);
				}
				else
				{
...
estimate_cardinality_with_sampling(double total_rows)
{
    ...
    appendStringInfo(&query,
                     "SELECT COUNT(*)::float FROM (SELECT %s FROM %s TABLESAMPLE BERNOULLI(%f)) p "
                     "WHERE p.%s %s '%s' < %f",
...
/*
 * Phase 6 Step 4: Stratified sampling counterpart of
 * estimate_cardinality_with_sampling. The data-side stratum_id column is
 * pre-populated by an external setup (KM20 layer assigned via ALTER TABLE +
 * COPY + UPDATE). At runtime we (1) load the per-stratum row counts once into
 * a process-local cache, (2) issue a single SPI query that draws an even
 * sample from each stratum (ORDER BY random() LIMIT s_i), filters by the
 * vector range predicate, groups by stratum, and returns the
 * Horvitz-Thompson sum as a single float8 row.
 *
 * Reproduces the Python estimator in phase6_layer_compare.estimate_stratified
 * exactly when stratum_sizes_cache matches the offline KM20 sizes.
 */
static void
load_stratum_sizes_cache(int n_strata)
{
    StringInfoData query;
    int ret;
    bool isnull;
    ...
    if (stratum_sizes_cached &&
        strcmp(stratum_sizes_cached_table, vector_table_name) == 0)
    {
        return;
    }

    memset(stratum_sizes_cache, 0, sizeof(stratum_sizes_cache));

    initStringInfo(&query);
    appendStringInfo(&query,
        "SELECT %s::int, count(*)::float8 FROM %s GROUP BY %s ORDER BY %s",
        stratified_table_column, vector_table_name,
        stratified_table_column, stratified_table_column);

    SPI_connect();
    ret = SPI_execute(query.data, true, 0);
    if (ret == SPI_OK_SELECT && SPI_processed > 0)
    {
        for (i = 0; i < SPI_processed && (int) i < VECTOR_MAX_STRATA; i++)
        {
            Datum sid_d = SPI_getbinval(SPI_tuptable->vals[i],
                                        SPI_tuptable->tupdesc, 1, &isnull);
            int sid = isnull ? -1 : DatumGetInt32(sid_d);
            Datum n_d = SPI_getbinval(SPI_tuptable->vals[i],
                                      SPI_tuptable->tupdesc, 2, &isnull);
            double n_i = isnull ? 0.0 : DatumGetFloat8(n_d);
            if (sid >= 0 && sid < VECTOR_MAX_STRATA)
                stratum_sizes_cache[sid] = n_i;
        }
        stratum_sizes_cached = true;
        strncpy(stratum_sizes_cached_table, vector_table_name,
                sizeof(stratum_sizes_cached_table) - 1);
        stratum_sizes_cached_table[sizeof(stratum_sizes_cached_table) - 1] = '\0';
        elog(LOG, "vector.so: stratum_sizes_cache loaded for %s, n_strata=%lu",
             vector_table_name, (unsigned long) SPI_processed);
    }
    else
    {
        elog(WARNING, "vector.so: stratum_sizes_cache load failed for %s (ret=%d)",
             vector_table_name, ret);
    }
    SPI_finish();
    pfree(query.data);
    (void) n_strata;  /* unused for now */
}

static double
estimate_cardinality_with_stratified_sampling(double total_rows, int n_strata)
{
    StringInfoData query;
    int ret;
    double est_total = 0.0;
    double cnt_total = 0.0;
    bool isnull;
    int i;
    int budget;
    int base;
    ...
    if (!stratum_sizes_cached ||
        strcmp(stratum_sizes_cached_table, vector_table_name) != 0)
    {
        load_stratum_sizes_cache(n_strata);
    }

    if (!stratum_sizes_cached)
    {
        is_sampling_active = false;
        elog(WARNING, "vector.so: stratified fallback (cache miss), returning total_rows");
        return total_rows;
    }

    budget = (int) sample_size;
    if (budget < 1)
        budget = 1;
    base = budget / n_strata;
    rem = budget - base * n_strata;

    initStringInfo(&query);
    appendStringInfoString(&query, "WITH sampled AS (");
    for (i = 0; i < n_strata; i++)
    {
        int s_i = base + (i < rem ? 1 : 0);
        if (s_i < 1)
            s_i = 1;
        if (i > 0)
            appendStringInfoString(&query, " UNION ALL ");
        appendStringInfo(&query,
            "(SELECT %d::int AS sid, %s FROM %s WHERE %s = %d ORDER BY random() LIMIT %d)",
            i, vector_column_name, vector_table_name,
            stratified_table_column, i, s_i);
    }
    appendStringInfoString(&query, "), filtered AS (SELECT sid FROM sampled WHERE ");
    appendStringInfo(&query, "%s %s '%s' < %f",
        vector_column_name, distance_function, vector_str, range_distance_value);
    ...
    appendStringInfoString(&query,
        ...
        "COALESCE(sum(cnt), 0)::float8 AS cnt_total FROM ("
        "SELECT sid, count(*)::float8 AS cnt, (CASE sid ");
    for (i = 0; i < n_strata; i++)
    {
        int s_i = base + (i < rem ? 1 : 0);
        double weight_i;
        if (s_i < 1)
            s_i = 1;
        weight_i = stratum_sizes_cache[i] / (double) s_i;
        appendStringInfo(&query, "WHEN %d THEN %f::float8 ", i, weight_i);
    }
    appendStringInfoString(&query, "ELSE 1.0::float8 END) AS weight "
    ...
    if (ret == SPI_OK_SELECT && SPI_processed > 0)
    {
        TupleDesc tupdesc = SPI_tuptable->tupdesc;
        HeapTuple tuple = SPI_tuptable->vals[0];
        Datum est_d = SPI_getbinval(tuple, tupdesc, 1, &isnull);
        if (!isnull)
            est_total = DatumGetFloat8(est_d);
        Datum cnt_d = SPI_getbinval(tuple, tupdesc, 2, &isnull);
        if (!isnull)
            cnt_total = DatumGetFloat8(cnt_d);
    }
    else
    {
        elog(WARNING, "vector.so: stratified SPI failed (ret=%d, processed=%lu)",
    ...
    /* cnt_total = 0 의 경우, BERNOULLI 의 L976~979 clamp 와 평행한 처리:
     * 평균 weight (total_rows / budget) 를 estimate 로 사용. */
    if (cnt_total == 0)
    {
        est_total = total_rows / (double) budget;
    }
