...
conn = psycopg2.connect(host='/tmp', port=5431)
cur = conn.cursor()
...
# %%
query_vector = str(deep[0].tolist()).replace('[', '{').replace(']', '}')
print(query_vector)

query = f""" /*+          indexscan(partsupp partsupp_deep_hnsw)      */
    SELECT
        ps_embedding <-> '{query_vector}' AS distance
    FROM
        partsupp
    ORDER BY
        ps_embedding <-> '{query_vector}'
    limit 200
"""
...
values = [str(0.75)] + query_vector.strip('{}').split(', ')
formatted_query = '{' + ', '.join(values) + '}'
...
query = f""" /*+          indexscan(partsupp partsupp_deep_hnsw)      */
    ...
    WHERE
        ...
        AND ps_embedding <<->> '{formatted_query}'
"""
