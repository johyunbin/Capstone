...
[SAFE VARIANT] conditional DELETE per dataset — 다른 table_name σ 보존.
원본 compute_stratum_sigma.py 의 unconditional DELETE FROM vector_stratum_sigma
가 이미 계산된 8M σ 등을 wipe 하는 문제를 수정.
...
서버에서 직접 실행: python3 -u compute_stratum_sigma_safe.py
"""
import psycopg
...
def main():
    ...
    print(f"[{kst()}] === RQ2 σ_i 사전 계산 [SAFE VARIANT] (vector_stratum_sigma 테이블) ===")

    # 테이블 생성 (DELETE 는 per-dataset 으로 이동 — 다른 table_name σ 보존)
    cur.execute("""
    ...
    """)
    conn.commit()
    print(f"[{kst()}] table ready (conditional DELETE per dataset — preserves other table_name σ)")

    # 시작 전 현재 σ table 상태 출력 (보존 검증용)
    cur.execute("SELECT table_name, COUNT(*) FROM vector_stratum_sigma GROUP BY table_name ORDER BY table_name")
    rows = cur.fetchall()
    if rows:
        print(f"[{kst()}] BEFORE state:")
        for r in rows:
            print(f"    {r[0]}: {r[1]} rows")
    else:
        print(f"[{kst()}] BEFORE state: empty table")

    for ds in DATASETS:
        print(f"\n[{kst()}] === {ds['name']} ({ds['table']}) ===")
        # (safe) per-dataset DELETE — 다른 table_name 보존
        # (atomic) DELETE + INSERT 한 transaction — 중간 실패 시 rollback (이전 σ 보존)
        cur.execute("DELETE FROM vector_stratum_sigma WHERE table_name = %s", (ds['table'],))
        # 주의: 여기서 commit 하지 않음 — 최종 INSERT 성공 후 같이 commit
        print(f"[{kst()}]   prepared DELETE for {ds['table']} (uncommitted, atomic with INSERT)")
