...
def cache_cluster_samples(ds):
    ...
    NPY-first dual-mode (W4 patch 5/7) — NPY 있으면 NPY 사용 (8M+ 빠름), 없으면 PG.
    """
    from pathlib import Path
    NPY_DIR_LOCAL = Path("/mnt/hdd0/home/capstone2026/cache/rq1")
    table = ds['table']
    npy_strata = NPY_DIR_LOCAL / f"{table}_strata.npy"
    npy_vecs = NPY_DIR_LOCAL / f"{table}_vectors.npy"
    if npy_strata.exists() and npy_vecs.exists():
        print(f"[{kst()}]   NPY mode: loading {table} from NPY...", flush=True)
        vecs = np.load(npy_vecs)
        strata = np.load(npy_strata)
        samples = {}
        sizes = {}
        sigmas = {}
        for sid in range(N_STRATA):
            idx = np.where(strata == sid)[0]
            sizes[sid] = int(len(idx))
            if len(idx) == 0:
                sigmas[sid] = 0.0
                samples[sid] = np.zeros((0, vecs.shape[1]), dtype=np.float32)
                continue
            cv = vecs[idx]
            centroid = cv.mean(axis=0)
            d2c = np.linalg.norm(cv - centroid, axis=1)
            sigmas[sid] = float(d2c.std()) if len(idx) > 1 else 0.0
            n_take = min(CACHE_PER_CLUSTER, len(idx))
            samples[sid] = cv[:n_take].astype(np.float32)
        total_rows = sum(s.shape[0] for s in samples.values())
        total_mb = sum(s.nbytes for s in samples.values()) / 1e6
        print(f"[{kst()}]   NPY cached {total_rows} rows ({total_mb:.1f} MB) — sizes={dict(list(sizes.items())[:3])}..., σ_avg={np.mean(list(sigmas.values())):.4f}", flush=True)
        return samples, sizes, sigmas
    print(f"[{kst()}]   PG mode: caching {ds['name']} cluster samples (LIMIT {CACHE_PER_CLUSTER} per cluster, fresh conn)...")
    samples = {}
    sizes = {}
    sigmas = {}
