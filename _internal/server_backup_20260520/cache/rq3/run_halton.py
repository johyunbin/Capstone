#!/usr/bin/env python3
"""
RQ3 Wave1 — Halton sequence quasi-random stratification 측정.

PCA 2D + Halton point N_STRATA 개로 nearest-neighbor stratum 부여.
Sobol 와 같은 QMC family이지만 prime base (2,3) radical inverse.

가설 H3-Hl: Sobol 과 비슷한 성능 — QMC family 동일 카테고리.
"""
...
from halton.halton_stratification import (  # noqa: E402
    assign_halton, cluster_size_summary, fit_halton,
)


def main():
    ap = argparse.ArgumentParser(description="RQ3 Wave1 — Halton quasi-random stratification")
    ap.add_argument("--out-prefix", default="rq3_halton")
    ...
    for ds in use_datasets:
        ...
        print(f"[{kst()}]   Halton PCA + halton point fit on {n_learn:,} samples")
        ...
        print(f"[{kst()}]   Halton fit elapsed: {learn_elapsed:.1f}s")

        stratum_ids = assign_halton(mapper, all_vecs)
        summary = cluster_size_summary(stratum_ids, n_clusters=N_STRATA)
        print(
            f"[{kst()}]   Halton cluster sizes: min={summary['min']}, "
            f"max={summary['max']}, max/min={summary['max_min_ratio']:.2f}"
        )

        rows = run_method_measurement(
            method_name="halton", all_vecs=all_vecs, stratum_ids=stratum_ids,
    ...
    save_parquet_meta(
        ...
        extra_meta={
            "method": "Halton quasi-random stratification (QMC, prime base 2/3)",
