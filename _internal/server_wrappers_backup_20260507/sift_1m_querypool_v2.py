#!/usr/bin/env python3
"""SIFT 1M querypool — long format (query_id × selectivity row, D_target/true_cardinality cols).
...
def main():
    print(f'[{kst()}] === SIFT 1M query pool (long format) ===', flush=True)
    ...
    sel_df.to_parquet(NPY_DIR / 'query_selectivity_sift_1m.parquet')
    print(f'[{kst()}]   saved query_pool_sift_1m.parquet ({len(qp_df)} rows)', flush=True)
    print(f'[{kst()}]   saved query_selectivity_sift_1m.parquet ({len(sel_df)} rows long format)', flush=True)
