#!/usr/bin/env python3
"""SIFT 8M Step 7 v2 — query pool 100q + d_target 5 sel + true_card.

v1 비효율 회피: 8M 전체 norm 을 query × sel = 500번 → query 당 1번 (100 passes).
"""
import sys, time
...
def main():
    print(f'[{kst()}] === SIFT 8M query pool v2 START ===', flush=True)

    all_vecs = np.load(NPY_DIR / f'{TABLE}_vectors.npy')
    custkeys = np.load(NPY_DIR / f'{TABLE}_custkeys.npy')
    print(f'[{kst()}]   loaded {len(all_vecs):,} × {all_vecs.shape[1]}', flush=True)
    ...
    print(f'[{kst()}]   sample for sel quantile: {SAMPLE_FOR_SEL:,}', flush=True)

    print(f'[{kst()}]   computing per-query 8M norm + sample quantile...', flush=True)
    rows = []
    t0 = time.time()
    ...
    out_df.to_parquet(NPY_DIR / 'query_selectivity_sift_8m.parquet')

    print(f'[{kst()}]   saved query_pool_sift_8m.parquet ({len(qp_df)} rows)', flush=True)
    print(f'[{kst()}]   saved query_selectivity_sift_8m.parquet ({len(out_df)} rows)', flush=True)
    print(f'[{kst()}] === SIFT 8M query pool v2 DONE ===', flush=True)
