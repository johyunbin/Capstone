#!/usr/bin/env python3
"""
RQ3 #15 — Spectral Clustering 측정.

KMeans 와 직교적 paradigm — graph Laplacian eigenvector. Nystrom 변형으로
large-N 적용 (학습 1% sample 에 spectral, all_vecs 는 nearest centroid).
"""
...
from spectral.spectral_clustering import (  # noqa: E402
    assign_spectral, cluster_size_summary, fit_spectral,
)


def main():
    ap = argparse.ArgumentParser(description="RQ3 #15 — Spectral Clustering")
    ap.add_argument("--out-prefix", default="rq3_spectral")
    ap.add_argument("--datasets", nargs="*", default=None)
    ap.add_argument("--n-queries", type=int, default=100)
    ap.add_argument("--learn-frac", type=float, default=0.005,
                    help="학습 sample 비율 (default 0.5%% — Spectral 은 O(N²) 라 작게)")
    ...
    print(f"[{kst()}] === RQ3 #15 — Spectral Clustering ===")
    ...
    for ds in use_datasets:
        ...
        print(f"[{kst()}]   spectral fit on {n_learn:,} samples (k={N_STRATA})")

        t_learn = time.time()
        with warnings.catch_warnings():
            warnings.simplefilter("ignore")
            mapper = fit_spectral(learn_samples, n_strata=N_STRATA, seed=args.learn_seed)
        learn_elapsed = time.time() - t_learn
        print(f"[{kst()}]   spectral fit elapsed: {learn_elapsed:.1f}s")

        stratum_ids = assign_spectral(mapper, all_vecs)
        summary = cluster_size_summary(stratum_ids, n_clusters=N_STRATA)
        print(
            f"[{kst()}]   spectral cluster sizes: min={summary['min']}, "
            f"max={summary['max']}, max/min={summary['max_min_ratio']:.2f}"
        )

        rows = run_method_measurement(
            method_name="spectral", all_vecs=all_vecs, stratum_ids=stratum_ids,
    ...
    save_parquet_meta(
        ...
        extra_meta={
            "method": "Spectral Clustering (Nystrom 변형)",
