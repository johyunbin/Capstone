...
| 5% | **+1.85%** | (단일 측정) | 3/5 |
| 1% (좁은 범위) | **+8.93%** | (단일 측정) | 4/5 |

50%, 30%, 10% 구간은 5-seed 반복 측정으로 신뢰구간이 확보되었습니다. 5%와 1%는 초기 Phase 6에서 단일 selectivity 측정으로 수행되어 CI 산출 대상이 아니지만, 이후 실험 5의 대조군 데이터와 gradient 분석에서 일관성이 확인됩니다.
