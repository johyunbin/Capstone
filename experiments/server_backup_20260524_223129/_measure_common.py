...
def fetch_all_vectors_safe(ds: dict, n_strata: int = N_STRATA):
    ...
    strata_npy = CACHE / f"{table}_strata.npy"

    # NPY-only fast path (option C): both vectors + strata NPY exist → no PG roundtrip
    if cache_npy.exists() and strata_npy.exists():
        t0 = time.time()
        all_vecs_raw = np.load(cache_npy)
        sid_by_pk = np.load(strata_npy).astype(np.int32)
        if len(sid_by_pk) == len(all_vecs_raw):
            ...
            print(f"[{kst()}]   fetched {all_vecs.shape[0]:,} × {all_vecs.shape[1]}d "
                  f"({all_vecs.nbytes / 1e6:.1f} MB) in {elapsed:.1f}s [npy-only]")
            return all_vecs, km20_sids
        else:
            print(f"[{kst()}]   strata npy size mismatch ({len(sid_by_pk)} vs {len(all_vecs_raw)}) — fallback")
...
def fetch_stratum_sigmas(ds: dict) -> dict[int, float]:
    """vector_stratum_sigma 테이블 (table_name, stratum_id) → sigma fetch.

    KM20 oracle σ_j (paper RQ2 Neyman/Anti-Neyman 영역). 사전 빌드된 테이블 사용.
    {stratum_id: sigma_j} dict 반환. table_name 매칭 0건이면 빈 dict.
    """
    table_name = ds["table"]
    conn = _connect()
    try:
        with conn.cursor() as cur:
            cur.execute(
                "SELECT stratum_id, sigma FROM vector_stratum_sigma WHERE table_name = %s",
                (table_name,),
            )
            rows = cur.fetchall()
    finally:
        conn.close()
    return {int(sid): float(sigma) if sigma is not None else 0.0 for sid, sigma in rows}


def neyman_alloc(sizes: dict[int, int], sigmas: dict[int, float],
                  budget: int = SAMPLE_SIZE,
                  n_strata: int = N_STRATA) -> np.ndarray:
    """Neyman allocation — n_h ∝ N_h × σ_h (이질 cluster 더 sampling).

    paper RQ2 narrative anchor: 분포 알 때 최소 분산 estimator.
    """
    sz = np.array([sizes.get(j, 0) for j in range(n_strata)], dtype=float)
    sg = np.array([sigmas.get(j, 0.0) for j in range(n_strata)], dtype=float)
    weights = sz * sg
    if weights.sum() <= 0:
        return equal_alloc(n_strata, budget)
    ...
    if extra > 0:
        ...
        idx = np.argsort(-s)[:abs(extra)]
        s[idx] = np.maximum(s[idx] - 1, 1)
    return np.maximum(s, 1)


def anti_neyman_alloc(sizes: dict[int, int], sigmas: dict[int, float],
                       budget: int = SAMPLE_SIZE,
                       n_strata: int = N_STRATA) -> np.ndarray:
    """Anti-Neyman — n_h ∝ N_h / σ_h (동질 cluster 더 sampling).

    Neyman 의 reverse — 보충성 stress test, RQ2 narrative robustness 검증용.
    """
    sz = np.array([sizes.get(j, 0) for j in range(n_strata)], dtype=float)
    sg = np.array([sigmas.get(j, 0.0) for j in range(n_strata)], dtype=float)
    sg_safe = np.where(sg > 0, sg, 1e-6)
    weights = sz / sg_safe
    if weights.sum() <= 0:
        return equal_alloc(n_strata, budget)
    f = weights / weights.sum() * budget
...
def bernoulli_estimate(samples, sizes, qvec, D, rng, budget=SAMPLE_SIZE,
                        n_strata=None, all_vecs=None):
    """paper §V-B Bernoulli random sampling — "전체에서 random sample".

    B1 (대조군) 정의: 전체 벡터에서 budget 만큼 무작위 추출. strata 와 무관.

    K10 B1 결함 fix (5/17): all_vecs 가 주어지면 strata 캐시
    (cache_cluster_samples_inmem 의 K 의존 cluster cap) 를 우회하고 전체 벡터를
    직접 모집단으로 사용. K 무관. all_vecs=None 이면 기존 strata-flatten 경로 유지
    (CaseB 의 est_b1 등 — 기존 동작 완전 보존).
    """
    if all_vecs is not None:
        # B1 paper-exact 경로: 전체 벡터 직접 random sample (K 무관)
        n = all_vecs.shape[0]
        s = min(int(budget), n)
        if s == 0:
            return n
        ...
        hits = int((d < D).sum())
        return hits * (n / s)
    # 기존 strata-flatten 경로 (CaseB est_b1 등 — 변경 없음)
    # K=10 fix (5/16): use samples dict keys instead of range(n_strata) to avoid
    # default N_STRATA mismatch when STRATA_K env mutates mc.N_STRATA after import.
...
def run_method_measurement(
    ...
    *,
    ...
    rows: list[dict] = []
    sigmas: dict[int, float] | None = None  # lazy fetch (neyman/anti만 필요)
    for mode in modes:
        if mode in ("neyman", "anti_neyman") and sigmas is None:
            sigmas = fetch_stratum_sigmas(ds)
            if not sigmas:
                print(f"[{kst()}]   ⚠️ {ds['table']} sigma 부재 — equal로 fallback")
        ...
        if mode == "bernoulli":
            ...
            alloc = neyman_alloc(sizes, sigmas) if sigmas else equal_alloc()
            row_mode = method_name + "_neyman"
        elif mode == "anti_neyman":
            alloc = anti_neyman_alloc(sizes, sigmas) if sigmas else equal_alloc()
            row_mode = method_name + "_anti"
        else:
