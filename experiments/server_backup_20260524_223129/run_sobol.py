#!/usr/bin/env python3
"""
RQ3 #19 — Sobol sequence quasi-random stratification 측정.

PCA 2D + Sobol point N_STRATA 개로 nearest-neighbor stratum 부여.
*결정론적* + low-discrepancy (uniform spread).

가설 H3-Sb: RANDOM20 보다 우수, Hilbert 와 trade-off (locality vs uniform).
"""
...
from sobol.sobol_stratification import (  # noqa: E402
    assign_sobol, cluster_size_summary, fit_sobol,
)


def main():
    ap = argparse.ArgumentParser(description="RQ3 #19 — Sobol quasi-random stratification")
    ap.add_argument("--out-prefix", default="rq3_sobol")
    ...
    for ds in use_datasets:
        ...
        print(f"[{kst()}]   Sobol PCA + sobol point fit on {n_learn:,} samples")
    ...
    save_parquet_meta(
        ...
        extra_meta={
            "method": "Sobol quasi-random stratification",
