...
### ⚠ 분모 붕괴 경계 (critical caveat)

RQ2 의 실측에서 일부 셀은 `KM20 − RANDOM20` 의 절대값이 seed 간 표준편차보다 작다:

| 데이터셋 | selectivity | KM20 | RANDOM20 | gap (분모) | 판정 |
|---|---|---|---|---|---|
| DEEP 1M | 50% | +1.64% | +2.20% | **−0.56%p** | 분모 음수 → Recovery Rate 의미 없음 |
| DEEP 8M | 5% | +0.55% | +0.20% | +0.35%p | 분모 < noise → 불안정 |
| SIFT 1.5M | 1% | −0.53% | −12.11% | +11.58%p | 사용 가능 |

**규칙**: `|KM20 − RANDOM20| ≤ 1%p` 또는 seed 간 분산의 1σ 이하인 셀에서는 Recovery Rate 를 보고하지 않고, **절대 Q-error 개선폭 (방법X − BERN)** 을 primary metric 으로 사용한다. 이는 metric non-identifiability 를 회피하기 위한 사전 규약이며, 본 규약은 측정 전 pre-register 되어 post-hoc cherry-picking 을 방지한다.
...
##### ⚠ H 만의 공정 비교 보정 — 2×2 Factorial 필수

H 는 다른 6 방법과 달리 **`hook_est` 반환 공식 자체 (가중 평균) 를 수정**하므로, 7-way 단순 비교는 "배분 방식 (allocation)" 효과와 "추정 공식 (estimator)" 효과가 교락 (confounded) 된다. 이를 분리하기 위해 H 는 다음 **2×2 factorial** 로 측정한다:

| | Uniform estimator (기존 `hook_est`) | IS estimator (가중 평균) |
|---|---|---|
| **KM20 allocation** | RQ2 기존 결과 (reference) | `KM20-IS` (신규 측정) |
| **H allocation (no partition)** | `H-uniform` (신규 측정) | `H-IS` (본 설계의 default) |

이 4 조합을 모두 측정하면:
- `H-IS − H-uniform` = estimator 공식 변경 효과 (단독)
- `KM20-IS − H-IS` = allocation 변경 효과 (estimator 공식 통제)
- `KM20-baseline − H-uniform` = RQ2 대비 H allocation 의 단독 효과

이로써 H 의 개선분을 allocation vs estimator 두 원인으로 분해할 수 있다. 본 factorial 측정은 W2 (5/5~5/11) 의 H 구현 시점에 반드시 선행되며, 기본 단순 7-way 비교에서는 H 를 "대표 메커니즘" 으로만 제시하고 factorial 결과를 appendix 에 본격 보고한다.
