...
벡터 증강 분석 쿼리(VAQ)는 벡터 유사도 검색과 관계형 분석을 결합하는 차세대 데이터 분석의 핵심 쿼리 유형이다. RAG 파이프라인, 추천 시스템, 멀티모달 검색 등에서 광범위하게 사용되고 있으나, 기존 RDBMS 쿼리 옵티마이저는 벡터 조건의 선택도(selectivity)를 정확히 추정하지 못하여 잘못된 실행 계획을 생성한다. pgvector는 벡터 조건의 선택도를 33.3%로 고정 추정하고, VBASE는 50%, DuckDB는 100%로 추정하는데, 실제 선택도는 0.001%에서 90%까지 극도로 다양하다.

Exqutor [1]는 ECQO와 Adaptive Sampling이라는 두 기법으로 카디널리티 추정 문제를 해결하여 pgvector에서 최대 1,000배, VBASE에서 10,000배, DuckDB에서 1.5~37배의 성능 향상을 달성하였다. 그러나 Exqutor는 벡터 조건의 구조 자체를 변환하여 성능을 개선하는 접근은 탐구하지 않았으며, VAQ의 플래닝 시간과 실행 시간을 분리하여 병목을 분석하지도 않았다.
...
구체적 목표는 (1) VAQ에서 Planning Time vs Execution Time 병목 구조를 정량적으로 프로파일링하고, (2) Cascaded Decomposition이 end-to-end latency에 미치는 영향을 측정하며, (3) Exqutor ECQO와의 상호보완성/대체성을 실험적으로 판별하는 것이다.
...
이 단계의 결과에 따라 2단계 실험의 초점이 결정된다. 플래닝이 유의미한 병목(Planning Time ≥ 전체 latency의 20%)이면 "계획 품질 개선 + 실행 가속" 양쪽을, 그렇지 않으면(Planning Time < 10%) "구조적 pre-filtering을 통한 실행 가속"에 집중한다.
...
- Phase 2a (12~16개 조합): 분해 임계값 4가지 × 비교 전략 4가지(Baseline / ECQO only / Cascade only / ECQO + Cascade). 필터 없이 고정, 캐싱은 Materialized CTE만 사용.
- Phase 2b (24~36개 조합): 유의미한 조합(latency 10%+ 차이)만 → 필터 4단계 × 캐싱 전략 3가지(CTE / Materialized CTE / Temp Table + Index)로 확장
- 측정: 각 조합 5회 실행, 첫 1회 워밍업 제외, 나머지 4회 중 최대값을 아웃라이어로 제거하고 3회 평균 사용. `pg_stat_statements` 리셋 후 단독 세션 실행
...
| Q-error [5] | max(estimated/actual, actual/estimated) |
...
2. **선택도 vs Q-error 곡선 + crossover point 맵** — 기존 추정이 틀리는 영역 시각화
3. **분해 임계값 × end-to-end latency 최적점 그래프** — Cascade의 sweet spot
4. **ECQO vs Cascade vs 결합 성능 비교표** — 보완/대체 관계 판별
5. **중간 결과 캐싱 전략 효과 비교** — CTE / Materialized CTE / Temp Table+Index 비교
6. **확장 실험 결과** (top-k, pgvector 0.8.0 등) — 3단계 선택 항목
7. **조건별 최적 전략 가이드라인** — 실무 적용 권장 사항
...
[2] J. Johnson, M. Douze, and H. Jégou, "Billion-scale similarity search with GPUs," *IEEE Transactions on Big Data*, vol. 7, no. 3, pp. 535-547, 2021.<br>
[3] V. Markl, V. Raman, D. Simmen, G. Lohman, H. Pirahesh, and M. Cilimdzic, "Robust Query Processing through Progressive Optimization," in *Proc. ACM SIGMOD*, 2004.<br>
[4] S. Gollapudi, N. Kini, V. Sivashankar, R. Krishnaswamy, H. Simhadri, et al., "Filtered-DiskANN: Graph Algorithms for Approximate Nearest Neighbor Search with Filters," in *Proc. WWW*, 2023.<br>
