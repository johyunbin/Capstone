...
RQ2는 "이론적 상한(upper bound)" — 분포를 완벽히 알 때 얼마나 좋아질 수 있는지. RQ3는 "현실적 해법" — 분포를 모를 때도 얼마나 근접할 수 있는지. 이 구조가 교수님의 2 트랙(aware/agnostic)과 정확히 대응된다.
...
- **주 데이터셋**: 석사분이 제공할 Exqutor 실험 데이터 (4월 둘째 주 수령 예정)
  - TPC-H/DS 기반 VAQ 벤치마크 + 벡터 임베딩 포함 예상
  - 수령 후 거리 분포의 skewness 프로파일링 먼저 수행
...
- **샘플링 구현**: Python (NumPy, SciPy) — Exqutor의 adaptive sampling 로직을 Python으로 재구현하여 동일 조건 비교
...
Latin Hypercube Sampling은 메인 실험 기법이 아닌 **관련 연구(Related Work)** 및 **이론적 배경**으로 다룬다:
- 1차원 거리값에 LHS를 적용하면 equal-frequency stratification과 사실상 동일
- 다만 다차원 확장(벡터 공간 직접 층화) 가능성은 향후 연구로 언급
- Exqutor가 구현하지 않은 이유(고차원 오버헤드)를 분석하여 기술
...
| 4/17~5/1 | 1단계(RQ1) Motivation 실험 | Skewness vs Q-error 분석 |
| 5/1~5/15 | 2단계(RQ2) + 3단계(RQ3) 본실험 | 전략별 비교 결과 |
| 5월 중 | **중간발표 + 중간보고서** | 발표자료, 보고서 |
| 5/15~6/15 | 추가 실험 + 분석 심화 + 최종 정리 | 최종 결과 |
| 6월 | **최종발표 + 최종보고서 + 전시회** | 최종 산출물 |
