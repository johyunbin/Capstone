...
작성: 2026-05-14 16:00 KST · 분량: 약 1700 줄 (v2 가독성 update: callout 박스 47 개 추가 + LaTeX 수식 → unicode + 핵심 수치 mark highlight + 박세은 / 채림님 quote 박스 + Neyman paradox danger 박스) · 회의 의견 8건 + 회의 질문 15+ 가지 답변 통합 + 6 핵심 method 부록 + 환각 disclosure + 약어 cheat sheet (§0.4)
...
!!! success "★ 본 연구 한 줄 요약"
    "벡터 분석 쿼리에서 데이터가 한쪽으로 쏠려 있을 때, 본 논문의 무작위 표집 (베르누이) 을 분포 정보 기반 표집으로 갈아끼우면 **<mark>Q-error 가 평균 10% 정도 줄어든다</mark>**. 결합 (산술 평균) 은 더 큰 정확도가 아니라 method 선택의 **안정성**이 가치다. **정확도와 자원 효율이 같은 method 군** (sparse_rp / chao_weighted / pca1d / hilbert_real / reservoir) 을 가리킨다."
...
문장에 표기된 노란 경고 박스 **"[검증 필요]"** 는 본 문서가 처음 정리한 framework 또는 실측 csv 직접 확인 미완 영역이다. 박광현 교수님 5/15 미팅 D-1 자문 권장 항목으로 별도 정리 (§7.2).

본 문서의 시각 표기 가이드 (PDF 렌더링 시 callout 박스 5 종):

- 🚨 빨강 (danger) — paradox / 모순 / 본질적 메커니즘 발견
- ✅ 초록 (success) — ★ 본 연구의 main finding / 시나리오 B 확정
- 💬 회색 (quote) — 회의록 verbatim 인용 (박세은 / 채림님)
- 💡 파랑 (info) — 저녁 회의 숙지 요점
- ⚠️ 노랑 (warning) — [검증 필요] 영역
...
!!! info "본 연구 핵심 질문"
    "분포 정보를 활용할 수 있다면 동일한 sample 수 (385 개) 로 **더 정확한 카디널리티 추정**이 가능한가? 그렇다면 **어떤 method 가, 얼마나, 어떤 자원으로?**"
...
!!! info "저녁 회의 숙지 요점 — 데이터셋"
    - **차원 D 는 데이터셋마다 다르다** — 채림님이 14:30 즈음에 "데이터셋 차원이 명시되어 있나" 라고 물었을 때, 본 연구는 **96 / 128 / 256 / 192 / 768 의 5 가지 차원**을 동시에 다룬다. WIKI 768d 는 BERT 임베딩이라 가장 고차원.
...
!!! info "저녁 회의 숙지 요점 — method 폐기 분류"
    - **폐기 사유 자체가 본 연구의 contribution** — 본 논문 Exqutor 는 "분포 정보를 활용해야 한다" 까지만 언급하고 구체적인 method 카탈로그를 제공하지 않았다. 우리는 **57 후보 → 40 폐기**의 분류 자체를 보고서에 명시한다.
...
!!! info "저녁 회의 숙지 요점 — sample 수 + 배분 방식"
...
!!! warning "[검증 필요] CLAUDE.md +3.74% 와 실측 표 부호 충돌"
    CLAUDE.md 에는 "+3.74%" mean gap 이라는 표현이 있는데, 이는 위 표의 **−6.35% / −1.95% 와 부호가 충돌**. fact extraction agent 의 실측 표는 위와 같다. 다른 측정 axis (예: 5 cell × 5 trial 의 다른 통계) 에서 +3.74% 가 산출됐을 가능성이 있다. 직접 verify 권장.
...
!!! danger "Neyman Paradox 정량"
    **Anti 1.540 < Prop 1.580 < <mark>Neyman 1.595</mark>** — 이론상 최적인 Neyman 이 실제로는 Proportional 에게 졌다. σ_j 의 범위가 **1.3 ~ 1.6 배로 좁고** 그룹 크기 N_i 의 CV (변동 계수) =0 인 환경 (모든 그룹 크기가 같다는 가정) 에서 Neyman 의 우위가 사라진다. 자세한 원인은 §6.

!!! warning "[검증 필요] 5-way 실측 csv 직접 확인 미완"
    5-way 5 cell × 5 trial 실측 csv 직접 확인은 미완. CLAUDE.md narrative base. 회의 Q7 (Neyman Q-error from→to) 의견 반영 향후 csv 직접 확인 권장. Anti-Neyman 1.540 이 negative control 인데도 best 인 anomaly 는 narrative 에 직접 포함하지 않음 (paradox 의미는 §6).
...
!!! success "★ 본 연구 main finding"
    **9-cell 평균: <mark>−10.17%</mark>** (단독 best, minibatch_partial)

해석 — SSN (balanced) 에서는 단독 대체가 약간 나빠지지만 (**+2.10%**), skew 환경 (DEEP / SIFT) 에서는 **<mark>−15 ~ −21% 의 큰 개선</mark>**. 평균 −10.17% 는 본 논문 자체 재현 변동 −4.3% 의 **2.4 배** 수준.

!!! success "저녁 회의 숙지 요점 — ★ 본 연구의 main finding"
    - **<mark>−10.17%</mark> 가 본 연구의 main finding 정량** — 5/27 발표 + 6/11 보고서의 main number.
    - **skew 와 balanced 의 차이** — 9 cell 평균 안에 +2.10% (SSN balanced) 가 섞여 있다. 본 연구의 정직성 갈래로 narrative 에 명시.
    - **paper 재현 변동 대비 2.4 배** — paper 자체를 재현해도 ±4.3% 측정 변동이 있다. −10.17% 는 이 변동의 **2.4 배**라 통계적으로 명확한 개선.
...
A2-Fig9 single cell: B1=1.5407, Centroid tuple CaseB=1.4271 → **<mark>−7.37%</mark>**

**α sweep (알파 결합 비율 sweep: B1 과 method 추정값을 섞을 때 method 쪽 비중을 0.3 / 0.4 / 0.5 / 0.6 / 0.7 로 바꿔가며 측정)** CaseB best (sparse_rp α=0.5): −6.58%

**전체 결합 측정 결과:**

- 492 짝지어 비교 중 **<mark>92.5%</mark> 가 단독 대체 (CaseA) 보다 정확** (455/492, p < 1e-45) [출처 handoff_v12 line 120 "paired CaseB < CaseA", 검증 필요]
...
!!! success "저녁 회의 숙지 요점 — ★ 시나리오 B 확정 (결합의 진짜 가치)"
    - **결합은 정확도 1 등이 아니라 안정성 1 등** — 단독 대체는 SSN 에서 나빠지지만 결합은 **<mark>92.5%</mark> 짝지어 단독 대체보다 우위**. 산업 적용 시 method 선택 실패의 보험 역할.
    - **α=0.5 산술 평균이 best** — 본 연구는 4 method 중 3 method 가 α=0.5 best 라 산술 평균을 default 로 선택. paper 적용 단순화 측면. U-shape sensitivity.
...
특히 reservoir (Vitter 1985) 는 메모리 **<mark>O(1) — 데이터 크기 N 과 무관</mark>** 인데도 결합 Δ% −9.25% 의 **anchor 수준 (대표 method 와 견줄 만한 수준)** 정확도. 모바일 / 임베디드 / 스트리밍처럼 메모리가 제약인 환경에 그대로 적용 가능.

!!! info "회의 의견 #6 반영 — 1 초 이내 검증"
    method 실행 시간 1 초 이내 검증은 fit (학습) 시간 기준은 **검증 완료** (sparse_rp 0.1 s, chao_weighted 0.5 s). total elapsed (전체 측정 1 trial) 는 100 ~ 107 초로, 이는 본 측정 환경의 query 실행 + I/O 포함 시간이라 method 자체의 추가 비용은 무시할 수준 (1 초 이내 검증 완료).

!!! success "저녁 회의 숙지 요점 — Pareto frontier"
...
!!! warning "[검증 필요] Neyman from→to 실측 csv 직접 확인"
    5-way 5 cell × 5 trial 실측 csv 직접 확인 미완. CLAUDE.md narrative base.
...
!!! warning "[검증 필요] query 시점 wall-clock 직접 측정 미완"
    query 시점 measurement 자체의 wall-clock 직접 측정은 미완. fit time 만 직접 측정.
...
!!! quote "채림님 14:26 발화"
    "single-table 측정 데이터를 multi-join 측정으로 carry-over 한다는 표현이 이해 안 됨. carry-over 가 무엇?"
...
!!! quote "채림님 14:27 발화"
    "본 논문이 베르누이 sample budget 385 를 정한 식 (Eq 1) 을 본 연구가 그대로 사용했나? 새로 구현했나?"
...
!!! quote "채림님 14:28 발화"
    "다중 테이블 측정에서 두 테이블이 단순 관계형 join 인지, 두 벡터를 합친 새로운 벡터 join 인지?"
...
!!! quote "채림님 14:31 발화"
    "벡터 유사도가 L2 distance 인가 cosine similarity 인가? 본 논문은 어떤 거고, 본 연구는 어떤 거?"
...
!!! warning "[검증 필요] distance metric 직접 확인"
    paper exact 측정 script 의 distance metric 명시 직접 확인 권장. CLAUDE.md base verified. 5/15 박광현 미팅에서 자문 권장.

#### Q5 (14:34): "K=20 SF=10 도 best 인지, 분포 모를 때 method 실시간/사전 계산?"

!!! quote "채림님 14:34 발화"
    "K=20 default 가 SF=10 환경에서도 best 인지 확인됐나? 분포 모를 때 method 가 실시간 계산인지 사전 계산인지?"
...
!!! quote "채림님 14:41 발화"
    "본 논문 §V-B Adaptive Sampling 이 single-table 도 적용 되는 건가? 본 연구 실험이 모두 join 환경인 건가?"
...
!!! quote "채림님 14:42 발화 — narrative 재정의 trigger"
    "데이터셋이 시스템에 들어온 다음 method 학습하는 거면 결국 분포를 학습한 거다. '분포 모른다' 라는 표현이 가능한가?"
...
!!! quote "채림님 14:44 발화"
    "데이터가 도착하기 전에 skew flag (e.g. '이 데이터는 high skew 다') 만 주어진 환경에서 본 연구의 method 가 적용 가능한가?"
...
!!! warning "[검증 필요] skew flag-only 환경 (본 연구 범위 밖)"
    skew flag-only 환경은 본 연구가 다루지 않은 영역. 회의에서 처음 제기.

#### Q9 (14:46): "σ_j 알면 k-means 없이 query 바로 가능 → RQ2 narrative 약해지지 않나?"

!!! quote "채림님 14:46 발화 — 비판적 통찰"
    "분산 σ_j 를 안다는 건 이미 그룹이 정의된 거다. 그런 환경에서 K-means + Neyman 비교한다는 narrative 가 약해지지 않나?"
...
!!! success "narrative 재정의 합의 #2"
    - **RQ2 = 이상적 천장** (실험에서만 가능, 산업 적용 어려움)
    - **RQ3 = 현실 환경** (학습 cost 포함, 산업 적용 가능)

본 연구의 main result 는 RQ3 다. RQ2 는 "천장이 어디인지" 의 reference. 본 연구 강점은 **<mark>RQ3 단독 best −10.17% 가 RQ2 천장 −9.61% (Proportional) 에 도달</mark>**했다는 점.

#### Q10 (14:52): "'들쭉날쭉한 정도' 의미 이해 안 됨, 클러스터링 = 비슷한 애들 묶음 아닌가?"

!!! quote "채림님 14:52 발화"
    "본 연구 narrative 의 '들쭉날쭉한 정도' 의미를 모르겠다. 클러스터링은 비슷한 애들끼리 묶는 거 아닌가?"
...
!!! quote "채림님 14:54 발화"
    "query 안에서 데이터가 비슷한가 라는 표현이 무슨 의미인지?"
...
!!! quote "채림님 14:57 발화 — ★ 본 회의 가장 본질적 의문"
    "벡터 query (L2 거리) 에서 클러스터링도 L2 base 라면, K-means 그룹화의 '비슷함' 과 query 안 응답 '비슷함' 이 같은 metric 으로 만들어지는 거 아닌가? 그러면 cluster 안의 query 응답이 자동으로 일관되고, σ_j 가 narrow 해지는 거 아닌가?"
...
- Query 벡터 `q` 에 대한 거리 `‖x − q‖` 가 점 `x` 마다 다르겠지만, **`x ∈ C_j` 라면 `x` 가 `c_j` 와 가까우므로 `‖x − q‖ ≈ ‖c_j − q‖` 의 근사**.
- → cluster 안 점들의 query 응답이 거의 yes 또는 거의 no.
- → cluster 안 응답 분산 `σ_j²` 가 작음.
- → cluster 간 `σ_j` 의 range 가 좁아짐 (1.3 ~ 1.6 배 narrow).

이로 인해:

!!! danger "Neyman Paradox 메커니즘 (★ 본 연구 발견)"
    ...
    - **paradox: Neyman 1.595 < Proportional 1.580 < Anti 1.540** — 이론상 최적이 실측에서 진다
...
!!! warning "[검증 필요] σ_j range 1.3~1.6 배 추정값 + paradox 일반화"
    5-way 5 cell × 5 trial 실측 csv 직접 확인 미완. narrative 의 σ_j range 1.3 ~ 1.6 배 추정값 검증 권장. paradox 의 일반화 가능성은 박광현 교수님 5/15 미팅 자문 권장.
...
!!! warning "[검증 필요] L0~L4 정보 수준 axis (본 문서가 처음 정리한 framework)"
    L0 ~ L4 axis 는 본 문서가 처음 정리한 framework. 본 연구 narrative 에 어떻게 반영할지는 박광현 교수님 5/15 미팅에서 자문 권장.
...
| **★ RQ3 단독 best (minibatch_partial 9 cell mean)** | **<mark>−10.17%</mark>** |
| RQ3 결합 best (Centroid tuple A2-Fig9) | −7.37% |
| RQ3 Pareto 자원 효율 best (reservoir) | −9.25% |

!!! success "본 연구의 강한 narrative (3 강점)"
    ...
    - **결합은 안정성 가치** — <mark>92.5% 짝지어 우위</mark>. 정확도 best 는 아니지만 method 선택 불확실 시 보험.
...
3. **Query 벡터 `q` 에 대한 거리 `‖x − q‖` 가 점 `x` 마다 다르겠지만, `x ∈ C_j` 라면 `x` 가 `c_j` 와 가까우므로 `‖x − q‖ ≈ ‖c_j − q‖` 의 근사가 성립**.
4. → 그룹 `C_j` 안 점들의 query 응답 (yes/no) 이 거의 yes 또는 거의 no.
5. → 그룹 안 응답 분산 `σ_j²` 가 작음 (응답이 거의 일관).

실측값: **<mark>σ_j 의 range 가 1.3 ~ 1.6 배로 좁다</mark>**.
...
> **`n_j ∝ N_j · σ_j`** (그룹 j 에 배분되는 sample 수 n_j 는 그룹 크기 N_j 와 그룹 표준편차 σ_j 의 곱에 비례)
...
- `σ_j` (그룹 표준편차) 의 range: **<mark>1.3 ~ 1.6 배</mark>**

→ `N_j · σ_j` 의 range 가 거의 `N_j` 의 range 와 비슷 (σ_j 는 weight 거의 안 줌).
...
!!! danger "저녁 회의 숙지 요점 — ★ Neyman paradox 의 본질 (꼭 알아둘 것)"
    - **Neyman paradox 의 원인은 K-means + L2 query 의 자연스러운 귀결**.
    - **σ_j range <mark>1.3 ~ 1.6 배</mark>가 narrow 의 정량값**.
...
- 단독 best CaseA: **<mark>−10.17%</mark>** (paper exact 변동 −4.3% 의 2.4 배)
...
- Sparse Random Projection (Li-Hastie-Church 2006). 행렬 `R ∈ {−1, 0, +1}^(D×k)` 의 sparse 형태로 `D` 차원 → `k` 차원 사영.
...
- `ε`-distortion 보존: `k = O(log N / ε²)` 차원으로 사영해도 pairwise distance 가 `(1 ± ε)` 보존.
...
- CaseB: −9.43% (안정적 우위)
- 학습 시간: **<mark>0.1 초</mark>** (Pareto best)
...
- CaseB: **<mark>−9.60%</mark>** (anchor 수준)
- 학습 시간: 약 0.5 초
...
- 메모리 O(K × N_per_stratum) = O(K × 19) ≈ 400 cells = **<mark>O(1) — 데이터 크기 N 과 무관</mark>**.
...
- 학습 시간: 0.1 ~ 0.5 초
- 메모리: O(N) (정렬용)
- K granularity: **<mark>K 변화에 둔감</mark>** (K=10 −10.86%, K=30 −11.26%)
...
작성: 2026-05-14 16:00 KST · 분량: 약 1700 줄 (v2 가독성 update: callout 박스 47 개 추가 + LaTeX 수식 → unicode + 핵심 수치 mark highlight + 박세은 / 채림님 quote 박스 + Neyman paradox danger 박스) · 회의 의견 8건 + 회의 질문 15+ 가지 답변 통합 + 6 핵심 method 부록 + 환각 disclosure + 약어 cheat sheet (§0.4) · 환각 0 원칙 (fact extraction agent 결과만 사용 + 검증 필요 영역 명시) · 5/14 저녁 긴급 회의 + 5/15 박광현 미팅 D-1 + 5/27 발표 + 6/11 보고서 공통 base
