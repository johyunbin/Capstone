...
★ **단독 대체 가능 method 후보 5-6 개 (5/14 정리)**:
- **sparse_rp**: 차원 축소 기반. 학습 0.1초로 cheap + 평균 -5 ~ -12% 개선.
- **chao_weighted**: 스트리밍 기반. 학습 0.5초 + 메모리 매우 적음 + 안정 정확도.
- **minibatch_partial**: 클러스터링 기반. 단독 best (-10.17%).
- **reservoir**: 스트리밍 기반. **메모리 O(1)** 로 자원 가장 적음.
- **hilbert / hilbert_real**: 공간 분할 기반. 학습 cheap + 정확도 강력.

본 5-6 단독 대체 후보의 자원 효율 분석은 5 장에서 다룬다.
