...
### 4.6.1 측정 코드 정합성 honest note — rng stream 공유 (5/24 v15 patch)

§4.2.2의 v15 portfolio 와 §5.6의 4-way engine 측정에서 사용된 `gen_latency_estimates.py` 의 `est_caseC` 산출은 a-side bernoulli rng 를 같은 (dataset, sf, sel, query_id) cell 의 강한 13 method stratified estimate 산출과 같은 generator(`np.random.default_rng(20260520)`)에서 share 한다. b-side bernoulli rng 는 별도 generator(`20260520 + 1_000_000` offset, measure_paper_exact.py `measure_case_c`의 seed pattern 과 동일) 로 분리되었으나, a-side stream 은 method 순서·중간 skip 발생 시 deterministic 재현성이 흔들릴 수 있음이 5월 24일 Codex xhigh 적대 검증에서 지적되었다. 본 portfolio 측정의 mean qe_trim 1.4620(18 셀) 결과는 single-run 실측으로 결론의 신뢰성에는 영향이 없으나, 차후 재측정 시 `rng_caseC_a` 를 별도 generator (예: `20260520 + 2_000_000` offset) 로 분리하여 method 순서 의존성을 끊는 fix 가 권장된다 (다음 세션 carry).
