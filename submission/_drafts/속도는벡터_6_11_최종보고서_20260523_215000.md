...
본 연구는 벡터 증강 분석 쿼리(vector-augmented analytical query, VAQ)의 카디널리티 추정에서 표본 선택 방식이 추정 정확도에 미치는 효과를 측정으로 검증한다. Exqutor 논문(arXiv:2512.09695v2) §V-B 적응적 표본 추출의 베르누이 표본 선택 단계 하나만 분포 인지 층화 표본 추출로 바꾸는 단일 개입을 두고, 16종 method × 5 데이터셋 × 5 조작변인의 측정 공간에서 베이스라인(B1)·완전 대체(CaseA, 음성 대조군)·결합(CaseB) 세 mode를 동시에 산출하는 3-way matched 1,508건 측정을 수행하였다. 결과는 결합이 베이스라인보다 카디널리티 추정의 Q-error를 89.1% (1,344/1,508건)에서 더 낮추며 중앙값 −4.38%의 개선을 가져옴을 보여 준다. 추정 정확도 향상이 엔진 단의 실행 시간으로 이어지는지 확인하기 위해 패치된 PostgreSQL의 카디널리티 자리에 결합 추정값을 주입한 12 cell × 16 variant × 15 rep = 2,880회의 end-to-end latency 측정을 추가로 수행하였으며, 정답 plan 회복은 베이스라인 7/12 대비 결합 13종 평균 148/156 = 94.9%로 회복되어 평균 5.67× 속도 향상이 관측되었다. paired Wilcoxon 검정 + Holm 보정 + Hedges' g + Cliff's δ + paired bootstrap 95% CI의 네 축 통계 검증 결과 베이스라인 대비 가속은 180/180 = 100% 모두 large effect로 견고하며, 결합 vs 베이스라인 latency는 13/168 = 7.7%에서만 유의하고 그중 86.9%가 small effect로 분포해 실행 시간이 동등함이 확인된다. 5월 23일 v14 캠페인에서 사전 등록 평균 비교군(CaseC = dual-Bernoulli ensemble, 9 셀 mean qe_trim 1.3729) 측정이 89% 우위의 메커니즘을 분포 인지 효과가 아닌 두 독립 추정량 평균의 통계 효과로 규명하였으며, 5월 24일 v15 portfolio 확장 측정(18 셀 mean qe_trim 1.4620, scale-dependent honest 변동 포함) 과 4-way engine latency 측정(B1·CaseA·CaseB·CaseC inject 변형 모두 |Δ%| ≤ 2% 동등) 에 이어 같은 날 v16 전수 측정(25 셀 × 3 selectivity × 3 K 평면의 95 tuple, BLOCKER E rng-stream-independent fix 적용, paper §V-B verbatim all_vecs 경로, mean qe_trim 1.306, paired vs B1 95/95 = 100% better with median Δ% −11.32%, paired vs CaseB 95/95 = 100% better with median Δ% −5.98%) 으로 controlled verification 의 결론이 측정 공간 전수에서도 강하게 재확인되었다 — 두 독립 추정량 평균이 분포 인지 결합보다 일관되게 더 정확하다는 음성 결과의 측정 전수 검증. 본 연구는 표본 선택 단계 하나의 개입만으로도 추정 오차의 개선 메커니즘과 엔진 단의 구조적 한계를 controlled verification 으로 동시에 규명한 negative·methodological 결과에 해당한다.
...
| P2 Spatial (PCA-reduced SFC + Vector Index) | pca2d_hilbert_xy2d · pca2d_zorder_morton · pca4_skilling_hilbert_approx · faiss_ivf | 4 |
| P3 Streaming | chao_weighted | 1 |
| P4 DimReduction | sparse_rp (Li 2006) · pca1d · rsvd · ica_fastica | 4 |
| P5 Classical Stratification (QMC 분리) | cum_sqrtf (Dalenius-Hodges 1959) · takeall_cumsqrtf | 2 |
| P6 Histogram / Quantization | rabitq_1bit_bucket · pca2d_equi_depth_grid | 2 |
| P5b Hashing (구 P9 폐지·이동) | md5_prefix_hash_bucket | 1 |

본 paradigm 표는 5/24 3-multi-AI audit (Claude+Codex+Gemini) 결과 정정 반영본 — 8 method rename + paradigm 2 곳 재분류 (P9 InfoTheoretic 폐지·P5b Hashing 신설 · P5 QMC → Classical Stratification). PCA-reduced 4 method (P2 Spatial) 는 학술 정직성 차원에서 paradigm 은 P2 유지하되 method 명칭에 `pca2d_`·`pca4_` prefix 를 명시한다. P1 Cluster (gmm·minibatch_partial) 는 PCA 환원 없이 원본 차원 적용으로 정합.
...
| **9 셀 평균** | — | **1.3729** | **1.5838** | **1.4723** | **−13.32%** | **−6.75%** |

9 셀 전부에서 사전 등록 통제군이 결합(CaseB)보다 정확하다(9/9 = 100%). 9 셀 평균 기준 통제군의 평균 추정 오차는 1.3729로 결합 1.4723보다 약 6.75% 더 낮으며, 베이스라인 대비로는 −13.32%로 §4.1의 결합 베이스라인 대비 우위(−3.06% 단순 산술 평균, −4.38% 중앙값) 보다 폭이 크다. 즉 method 자리에 분포 인지 method를 두는 결합 방식보다 또 다른 무작위 베르누이를 두는 통제군이 더 정확하다 — method(분포 인지)가 두 독립 추정량 평균 위에 양의 효과를 더하기는커녕 미세한 음의 잡음을 추가한다는 의미다.
...
### 4.2.3 사전 등록 통제 측정 전수 — 5월 24일 95 (cell × sel × K) tuple (v16)

§4.2.1 9 셀과 §4.2.2 18 셀 portfolio 는 모두 selectivity = 0.01 (또는 A4-sel 만 0.001) · K = 20 default 한 축에서 측정한 결과다. v13 정본 (B1·CaseA·CaseB 1,508 셀 matched) 의 측정 scope 와 완전히 일치시키기 위해, 5월 24일 본 캠페인은 25 cell sub × selectivity{0.001, 0.01, 0.10} × K{10, 20, 30} 평면의 v13 row 가 실재하는 95 (cell × sel × K) tuple 전수에 대해 dual-Bernoulli ensemble 측정을 수행하였다. 측정 hyperparameter 는 §4.2.1·§4.2.2 와 동일(trials=10·n_queries=1000·N=385) 이며, K 는 tuple 별 10/20/30 으로 override 한다.

본 측정은 §4.6.1 Codex BLOCKER E 적대 검증에서 지적된 rng stream 공유 결함을 fix 한 코드로 진행되었다 — `est_b1·est_caseC_a·est_caseC_b` 각자 별도 seed offset (20260520 + 4M / 2M / 3M) 로 분리하여 method estimate stream 의존성을 끊었으며, `measure_case_c` 의 `K_override` 는 v13 paired row 와의 매칭용 metadata 로만 기록하고 실제 cluster cache 는 항상 mc.N_STRATA=20 default 로 build, bernoulli sampling 은 `all_vecs` 직접 random sample 의 paper §V-B verbatim 경로(strata-independent) 를 사용한다. 따라서 v16 측정은 method 순서·개수 변경에도 deterministic 하고, 같은 (cell, sel) tuple 의 K=10/20/30 결과는 CaseC 가 본질적으로 K-independent 임을 정확히 반영하여 동일한 qe_trim 을 산출한다(K 별 차이는 paired vs B1·CaseB Δ% 에서만 나타난다 — B1·CaseB 가 method 의 strata K 의존).

95 tuple 전수 측정의 핵심 결과는 다음과 같다.

| 지표 | 값 | 비고 |
|---|---:|---|
| 전수 측정 tuple 수 | 95 / 95 (0 FAIL) | sf=100 32 · sf=10 37 · sf=1 26 · 2h 5min wall (3 병렬) |
| v16 CaseC mean qe_trim (95 tuple) | **1.306** | std 0.129 · min 1.105 · max 1.488 · median 1.366 |
| **v16 vs B1 paired better%** | **95 / 95 = 100.0%** | 모든 tuple 에서 통제군이 베이스라인보다 정확 |
| v16 vs B1 paired median Δ% | **−11.32%** | mean −10.05% · q25 −13.09% · q75 −4.60% |
| **v16 vs CaseB paired better%** | **95 / 95 = 100.0%** | 모든 tuple 에서 통제군이 결합보다 정확 |
| v16 vs CaseB paired median Δ% | **−5.98%** | mean −6.08% (v14 9 셀 −6.74% 와 매우 일관) |
| sf 별 qe_trim 평균 | sf=1 1.296 (n=26) · sf=10 1.301 (n=37) · sf=100 1.320 (n=32) | scale 영향 작음 |
| sel 별 qe_trim 평균 | sel=0.001 1.410 (n=28) · sel=0.01 1.368 (n=40) · sel=0.10 1.107 (n=27) | sel 증가 시 단조 감소 |
| K_meta 별 qe_trim 평균 | K=10 1.339 (n=12) · K=20 1.295 (n=71) · K=30 1.339 (n=12) | K=10/30 동일 (CaseC K-independent 의도 정합) |

세 가지 결정적 관찰이다. 첫째, 통제군 CaseC 가 모든 95 tuple 에서 베이스라인 B1 보다 추정 오차가 작다 (95/95 = 100%, 중앙값 −11.32% 개선). 이는 §4.1 의 결합 CaseB 가 B1 대비 89.1% better (중앙값 −4.38%) 보다 훨씬 강한 우위로, 통제군 = 두 독립 베르누이 추정량 평균이 분포 인지 결합보다 더 일관되고 더 큰 개선을 만든다는 §4.2.1·§4.2.2 결론이 측정 공간 전수 (95 tuple) 에서 재확인된다. 둘째, 통제군이 결합 CaseB 보다 모든 95 tuple 에서 더 정확하다 (95/95 = 100%, 중앙값 −5.98%). 즉 method (분포 인지) 를 method 자리에 두는 결합 방식보다 또 다른 무작위 베르누이를 두는 통제군이 일관되게 더 정확하다 — method 가 두 독립 추정량 평균 위에 양의 효과를 더하지 못한다는 결정적 음성 결과의 측정 전수 재확인이다. 셋째, sel·K·sf 평면에서 sel 효과가 가장 두드러진다 (sel 증가 시 mean qe_trim 단조 감소: 1.410 → 1.368 → 1.107). 큰 selectivity 에서 표본 안 적중 행 수가 증가하여 분산이 감소하는 표준적 효과로, §4.5 의 selectivity 효과 narrative 와 정합한다.

산출 parquet 은 `_internal/cache/rq3/paper_exact_v16_summary_20260524_122419/v16_full95_paired.parquet`, 셀·sel·K tuple 별 trial 디테일은 `paper_exact_v16_full95_20260524_122419/*_CaseC_sel{sel}_K{K}.json` (95 파일) 에 보관한다. 측정 figure 는 `experiments/figures/보고서_6_11/v16/fig1_qe_heatmap_sel{0.001,0.01,0.1}.{png,pdf}` (셀 × K heatmap × 3 sel) · `fig2_delta_vs_B1.{png,pdf}` (paired Δ% distribution) · `fig3_qe_by_sf.{png,pdf}` (sf 별 box) 에 carry.

![그림 4-2A. v16 95 tuple paired Δ% vs B1](../../experiments/figures/보고서_6_11/v16/fig2_delta_vs_B1.png)

![그림 4-2B. v16 sf 별 qe_trim 분포](../../experiments/figures/보고서_6_11/v16/fig3_qe_by_sf.png)

**그림 4-2A·B.** v16 95 tuple 전수 측정. (A) paired Δ% vs B1 — 95/95 = 100% 음수, 중앙값 −11.32%. (B) sf 별 qe_trim 분포 — sf=1·10·100 의 mean 차이 작음 (1.296/1.301/1.320).

### 4.3 method·paradigm 분석

method별 결합 대 대조군 비교를 중앙값 Δ% 오름차순으로 정리하면 다음과 같다.

| Method (정정 후 명칭) | Paradigm | n | better% | 중앙값 Δ% | 평균 Δ% (이상치 제외) |
|---|---|---:|---:|---:|---:|
| chao_weighted | P3 Streaming | 95 | 100.0% | −6.22% | −6.30% |
| **pca2d_hilbert_xy2d** (구 hilbert_real) | P2 Spatial (PCA-reduced) | 95 | 98.9% | −5.91% | −6.54% |
| **pca4_skilling_hilbert_approx** (구 skilling_hilbert) | P2 Spatial (PCA-reduced) | 94 | 100.0% | −5.75% | −6.34% |
| ica_fastica | P4 DimReduction | 94 | 100.0% | −5.69% | −6.13% |
| pca1d | P4 DimReduction | 94 | 97.9% | −5.55% | −6.05% |
| **pca2d_zorder_morton** (구 zorder_morton) | P2 Spatial (PCA-reduced) | 94 | 98.9% | −4.89% | −5.87% |
| **md5_prefix_hash_bucket** (구 hyperloglog) | P5b Hashing (구 P9 폐지) | 95 | 100.0% | −4.58% | −5.75% |
| cum_sqrtf | P5 Classical Stratification | 94 | 97.9% | −4.53% | −5.14% |
| **takeall_cumsqrtf** (구 lavallee_hidiroglou) | P5 Classical Stratification | 94 | 94.7% | −4.40% | −4.82% |
| sparse_rp (Li 2006) | P4 DimReduction | 95 | 84.2% | −4.37% | −3.69% |
| rsvd | P4 DimReduction | 94 | 91.5% | −4.10% | −4.36% |
| minibatch_partial | P1 Cluster | 94 | 79.8% | −3.58% | −2.52% |
| **rabitq_1bit_bucket** (구 rabitq_strat) | P6 Histogram/Quantization | 94 | 84.0% | −3.56% | −2.67% |
| **pca2d_equi_depth_grid** (구 mhist2) | P6 Histogram/Quantization | 94 | 88.3% | −3.41% | −3.16% |
...
paradigm 수준에서도 같은 결이 나타난다(P3 Streaming −6.22%·P4 DimReduction −4.72%·P2 Spatial PCA-reduced −4.66%·P5b Hashing −4.58%·P5 Classical Stratification −4.43%·P6 Histogram/Quantization −3.52%·P1 Cluster −1.40%; paradigm 라벨은 5/24 3-multi-AI audit 결과 정정 carry). method별 가장 강한 개선 8건은 모두 selectivity 0.01에서 나왔다(A1-SSN pca2d_hilbert_xy2d −13.60%·pca4_skilling_hilbert_approx −13.45% 등, 다중비교 보정 p값 0.0028 수준). 다만 **상위 1·2위 (pca2d_hilbert_xy2d·pca4_skilling_hilbert_approx) 와 6·7·14위 (pca2d_zorder_morton·md5_prefix_hash_bucket·pca2d_equi_depth_grid) 의 우위는 고차원 공간 곡선이나 정보 이론의 효과가 아니라 PCA 분산 요약 + 후속 알고리즘 hybrid 의 효과** 로 해석해야 한다 — 명칭의 PCA prefix 가 이를 명시한다 (§4.7 audit 결과 carry).
...
### 4.6.1 측정 코드 정합성 honest note — rng stream 공유 (5/24 v15→v16 patch · fix 완료)

§4.2.2의 v15 portfolio 와 §5.6의 4-way engine 측정에서 사용된 `gen_latency_estimates.py` 의 `est_caseC` 산출은 a-side bernoulli rng 를 같은 (dataset, sf, sel, query_id) cell 의 강한 13 method stratified estimate 산출과 같은 generator(`np.random.default_rng(20260520)`)에서 share 하였다. b-side bernoulli rng 는 별도 generator(`20260520 + 1_000_000` offset) 로 분리되었으나, a-side stream 은 method 순서·중간 skip 발생 시 deterministic 재현성이 흔들릴 수 있음이 5월 24일 Codex xhigh 적대 검증에서 BLOCKER E 로 지적되었다.

본 세션(5/24 오전)은 BLOCKER E fix 를 적용하였다. 4 개의 generator 를 완전 분리한다 — `rng = default_rng(20260520)` (method estimate 전용), `rng_b1 = default_rng(20260520 + 4_000_000)` (B1 baseline a-side), `rng_caseC_a = default_rng(20260520 + 2_000_000)` (CaseC dual a-side), `rng_caseC_b = default_rng(20260520 + 3_000_000)` (CaseC dual b-side). 그리고 `est_caseC` 산출 시 a-side 가 `est_b1` 을 재사용하던 기존 패턴을 제거하고 `est_caseC = (est_caseC_a + est_caseC_b) / 2.0` 로 자체 dual draw 평균을 사용한다. 따라서 method 순서·개수 변경에도 `est_b1·est_caseC` 가 deterministic 하다.

§4.2.3 v16 95 tuple 전수 측정은 fix 된 코드로 진행되며, §4.2.1·§4.2.2 의 carry 측정(v14·v15)은 single-run 실측으로 결론의 신뢰성에 영향이 없다(rng 공유는 분포가 아닌 재현성 결함이므로, 이미 산출된 결과의 통계 분포는 한 valid 실현이다). v15 portfolio (qe_trim 1.4620 · 18 셀) 와 v16 전수 (qe_trim 측정 후 §4.2.3 채움 · 95 tuple) 의 직접 비교는 §4.2.3 carry.

또한 v16 측정 시 Codex BLOCKER E 재 review 가 두 가지 추가 결함을 발견하였다 — (1) `measure_case_c`의 K_override 가 `km20_sids` 를 그대로 두고 `n_strata` 만 바꿔서 K=10 시 cluster 0~9 만 cache 하고 10~19 row 가 누락되었던 결함, (2) `bernoulli_estimate` 호출 시 `all_vecs` 미전달로 strata-flatten 경로 (cluster cap 500 under-represent corr -0.98) 를 따랐던 결함. 본 세션은 두 결함 모두 fix 하여 v16 측정은 (a) K_override 를 v13 paired row 와 매칭용 metadata 로만 기록하고 실제 cluster cache 는 항상 mc.N_STRATA=20 default 로 build, (b) `bernoulli_estimate` 호출에 `all_vecs` 전달로 paper §V-B verbatim 전체 벡터 직접 random sample 경로를 사용한다. 결과적으로 v16 의 같은 (cell, sel) tuple 의 K=10/20/30 측정 결과는 K-independent 동일 분포 (rng 동일·sampling K 동일) — paired vs B1/CaseB Δ% 만 K 별로 다른 의미를 갖는다(B1·CaseB 는 method 의 strata K 의존).

후속 carry item (다음 세션) 은 다음과 같다: (i) `measure_case_c` 의 query-sel 매핑 miss 시 silent fallback (TPC_H_THRESHOLD·total_rows*sel) 대신 raise — full95 전수 측정 시 결과 오염 차단; (ii) `gen_latency_estimates.py` 의 concat dataset table prefix 가 `partsupp_{combo}_{sf}` 인데 실제 NPY artifact 는 `partsupp_{combo}_concat_{sf}` 로 mismatch — concat fast-path 가 안 잡혀 fetch 시 실패 가능; (iii) `measure_offline_casec_portfolio` 의 `DEFAULT_PORTFOLIO` 가 18 cell 인데 KNOWN_CELLS 는 25 (A9·A10·A11 concat 7 추가) — wrapper portfolio launch 시 concat 7 cell 제외 (본 v16 측정은 95 tuple full launcher 가 별도로 정확히 처리).
...
- **method 명칭 정직성 (5/24 3-multi-AI audit 결과 정정)**: 본 연구에 사용한 16 method 중 8 method 가 학술 명칭과 실제 구현 사이에 부분 불일치를 보인다. (1) `hilbert_real` 은 **PCA 2D 환원 후 Wikipedia xy2d 진짜 2D Hilbert curve algorithm** 을 적용한 구현으로 고차원 Hilbert curve 가 아니다 (rename 권고: `pca2d_hilbert_xy2d`); 동일한 PCA 환원 시뮬레이션이 `skilling_hilbert` (PCA 4D + Skilling 알고리즘 근사, rename `pca4_skilling_hilbert_approx`), `zorder_morton` (PCA 2D + Morton bit-interleave, rename `pca2d_zorder_morton`), `mhist2` (PCA 2D + equi-depth grid, MaxDiff 미사용, rename `pca2d_equi_depth_grid`) 에도 해당된다. 따라서 본 §4.4 ranking 의 1·2위는 고차원 공간 곡선의 효과가 아니라 **PCA 분산 요약 + 공간 채움 곡선 hybrid 의 효과** 로 해석해야 한다. (2) `hyperloglog` 는 Flajolet 2007 HyperLogLog 의 카디널리티 추정 기능을 사용하지 않고 md5 hash leading prefix bits 만 partitioning 에 사용하므로 (rename `md5_prefix_hash_bucket`, paradigm 재분류 P9 InfoTheoretic → P5 Hashing). (3) `lavallee_hidiroglou` 는 Lavallée-Hidiroglou 1988 의 핵심인 Neyman σ_h allocation 을 적용하지 않고 take-all + cum-√f 만 사용한다 (rename `takeall_cumsqrtf`). (4) `rabitq_strat` 은 Gao-Lin 2024 의 full RaBitQ codebook 가 아닌 1-bit sign partition only (rename `rabitq_1bit_bucket`). (5) `sparse_rp` 의 1/√D 희소 무작위 투영 변형은 Li, Hastie, Church (2006) `Very Sparse Random Projections` 가 정본 출처 (Achlioptas 2003 표명 정정, §9 [14]). 측정 수치 자체는 유효하나 본 보고서·발표·외부 공유 자료에는 정정된 명칭과 PCA 환원·approximation 사용 명시를 carry 한다.
- **v16 CaseC vs B1 우위의 두 효과 분리** (5/24 Gemini Deep Think 적대 검증): §4.2.3 의 v16 CaseC vs B1 paired median Δ% −11.32% 는 (a) 두 독립 베르누이 추정량 평균에 따른 분산 감소 (일반 앙상블 효과, B1 단일 표본 대비 표본 용량 2배) 와 (b) 같은 앙상블 조건 하 method (분포 인지) 의 유효성 검정 — 두 효과가 함께 들어 있다. CaseC vs CaseB paired median Δ% −5.98% 는 동일 앙상블 조건 하 비교라 (b) 분포 인지가 양의 효과를 더하지 못한다 (오히려 미세 음의 잡음) 는 본 연구의 핵심 음성 결과를 정확히 집계한다. B1 대비 −11.32% 를 단순히 분포 인지 효과로 해석하지 말고 (a) 와 (b) 의 합으로 분해하여 읽어야 한다.
- **v16 CaseC K-independent 설계의 paired 비교 fairness 한계** (5/24 Gemini Deep Think 적대 검증): §4.2.3 의 v16 CaseC 측정은 paper §V-B verbatim dual-Bernoulli (cluster 무관 전체 벡터 random sample) 이라 본질적으로 K-independent 다 — 같은 (cell, sel) 의 K=10/20/30 tuple 의 qe_trim 이 동일하다. 그러나 v13 정본의 B1·CaseA·CaseB 는 method 의 strata K (cluster 개수) 에 의존하므로, K=10 tuple paired vs B1/CaseB Δ% 는 "K=20 고정 CaseC vs K=10 의존 B1/CaseB" 의 비대칭 비교가 된다. 결과 해석 시 K_meta=20 tuple (n=71) 의 paired Δ% 가 가장 fair 한 비교이며, K_meta=10/30 (각 n=12) 의 Δ% 는 strata-asymmetry 가 포함된 보조 지표로 본다. 본 한계는 향후 대조군 CaseC 의 sampling K 를 실험군과 동기화한 추가 측정 (K_caseC ∈ {10, 20, 30}) 으로 보완할 수 있다 (다음 세션 carry).
- **v14 → v15 → v16 평균 qe_trim 절대 수치 비교 caveat** (5/24 Gemini Deep Think 적대 검증): §4.2.1·§4.2.2·§4.2.3 의 mean qe_trim (1.373 → 1.462 → 1.306) 의 절대 수치 변화는 측정 cell 구성·sf 분포·sel 분포가 모두 다른 세 batch 의 비교로 algorithm 개선 효과로 직결될 수 없다 (selection bias). v16 의 1.306 이 v14 의 1.373 보다 낮은 것은 (a) v16 가 sf=1·10 작은 데이터셋 비중이 더 크고 (b) sel=0.1 큰 selectivity tuple 27건이 포함되어 큰 sample 효과로 평균이 낮아진 것이며, v16 의 paper-exact all_vecs 경로 도입은 측정 정확도 향상이 아닌 strata-flatten cap (cluster 500 cap 큰 cluster under-represent corr −0.98) 결함 해소로 측정 정합성·신뢰도 확보로 해석한다. 동일 9 셀 통제 비교는 §4.2.1 vs §4.2.3 의 같은 셀 subset (A1-DEEP·SIFT·SSN, A2-Fig7·Fig9, A4-sel, A5-scale-sf1·10·100, sel=0.01·K=20) 9 tuple 의 paired Δ% 가 fair 한 비교이며 다음 세션 carry.
- **Codex BLOCKER E re-review 후속 carry item 3종** (5/24): (i) `measure_case_c` 의 query-sel miss 시 silent fallback 대신 raise 도입 — full95 전수 측정 시 결과 오염 차단; (ii) `gen_latency_estimates.py` 의 concat dataset table prefix mismatch (`partsupp_{combo}_{sf}` 대 실제 `partsupp_{combo}_concat_{sf}`) fix; (iii) `measure_offline_casec_portfolio` 의 `DEFAULT_PORTFOLIO` 가 18 cell 인데 KNOWN_CELLS 25 — wrapper portfolio launch 시 concat 7 cell 누락. 본 v16 95 tuple 측정은 별도 `measure_offline_casec_full.py` launcher 가 95 tuple 전수를 정확히 처리하여 영향 X. 세 carry item 은 다음 세션 측정에서 fix.
...
두 anchor는 유의 비율과 효과크기 양 측면에서 정반대 분포를 보인다. Hedges' g는 짝지은 차이의 표준화 효과크기로 작은 표본 보정 j=1−3/(4n−5)을 적용(n=15, paired df=n−1; 코드 `analyze_latency.py:184` carry), Cliff's δ는 15회 짝 중 한쪽이 빠른 횟수의 차이 비율로 ±1.0은 일관된 우열을, bootstrap CI는 짝지은 차이 평균의 부트스트랩 95% 신뢰구간이다.
...
12 cell 최종 결과(5/24 04:03 KST · q3-qid2 한 cell 은 watchdog SIGTERM 영향으로 1차 측정 누락 → 재측정 후 12 cell 완성)는 다음을 보여 준다 — **CaseC vs B1 paired Δ% mean = +0.30%** (median +0.11%, std 1.22, 5/12 cell faster · 7/12 slower → 동전 던지기), **CaseA/mean vs B1 paired Δ% mean = −0.38%** (median −0.25%, std 2.66, 7/5), **oracle vs B1 −0.44%** (median +0.16%, std 2.75, 6/6 — 정답 plan 이미 회복된 상태의 자연 노이즈). 17 inject variant 전체(B1 제외) 의 cell-level paired Δ% 범위는 mean **−1.12% (CaseB/rabitq_strat, 가장 빠른 결합) ~ +1.09% (CaseB/ica_fastica, 가장 느린 결합)** 로 모두 |Δ%| ≤ 1.12% 안에 모인다. 베이스라인이 B1 대비 mean +409.7%·median +477.5%로 4~5× 느린 격차는 4-way 측정 전반에서 동일하게 유지된다. 즉 §5.4의 "결합 vs 베이스라인 latency 동등" 결론이 음성 대조군 CaseA(완전 대체) 와 평균비교 통제군 CaseC(dual-Bernoulli)로 확장된 평면에서도 모든 inject variant 의 동등성으로 재확인된다 — **추정 정확도 향상이 엔진 latency 의 차이를 만들지 못한다는 본 연구의 negative·structural 결과가 4-way로 한 번 더 검증된 것**이다. injection_fired rate 17 variant × 12 cell = 204/204 = 100% 로 모든 inject 가 pgvector vector.c 패치에서 정확히 발동되었음이 확정된다. variant 별 paired Δ% boxplot·variant 간 cell 별 trim latency 분포는 figure `experiments/figures/4way_latency_v15/fig1_paired_delta.{png,pdf}` + `fig2_variant_latency.{png,pdf}` 에 정리하며, 본 § 의 raw 측정은 `_internal/cache/rq3/latency/phase2_4way_final_<TS>/phase2_4way_summary.md`·`long.parquet`·`cell_summary.parquet`·`paired_summary.parquet` 에 carry된다.
...
|---|---|---|
| Type 1 (소규모 단일 sf=1) | chao_weighted, pca2d_hilbert_xy2d | 정확도 상위이면서 소규모 fit 비용 부담이 작음 |
| Type 2 (중규모 단일 sf=10) | chao_weighted, pca1d | 효과 일관성과 중간 수준 fit 비용 |
| Type 3 (대규모 단일 sf=100 저~중차원) | chao_weighted, pca2d_hilbert_xy2d, pca1d | K=20 안정 구간·정확도 상위 |
| Type 4a (다중 192~288차원) | pca2d_hilbert_xy2d, ica_fastica, pca1d | 다중 벡터 개선 비율 상위 |
| Type 4b (다중 864차원 이상) | pca2d_hilbert_xy2d, pca1d (클러스터링 회피) | 864차원 군집화 파탄(§4.7) |
