...
K granularity 측정에서 K=10/20/30을 모두 보유한 cell은 여덟 개다. 이 가운데 A1-DEEP·A2-Fig7·A2-Fig9·A5-scale-sf1·A5-scale-sf10·A5-scale-sf100 여섯 cell은 sel=0.01 조건에 K=10/30을 더한 전용 K-sweep이고, A1-SIFT·A1-SSN 두 cell은 full grid(3 selectivity × 3 K) 측정의 일부로 K=10/30을 함께 가진다. cell 구성을 통제한 깨끗한 비교를 위해, 전용 K-sweep 여섯 cell × sel=0.01 부분집합을 K별로 집계한다.
...
(주의: 위 비교는 전용 K-sweep 여섯 cell × sel=0.01 부분집합에 한정한 깨끗한 비교다. 전체 portfolio를 K별로 단순 집계하면 K=20이 25개 cell 전체를, K=10·K=30은 여덟 cell만 담아 cell 구성이 달라 직접 비교가 성립하지 않는다. cell 구성을 통제한 부분집합이 K granularity의 정당한 비교다.)
