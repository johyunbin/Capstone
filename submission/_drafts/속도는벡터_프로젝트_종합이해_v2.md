...
작성: 2026-05-14 16:00 KST · 분량: 약 1400 줄 · 회의 의견 8건 + 회의 질문 15+ 가지 답변 통합 + 6 핵심 method 부록 + 환각 disclosure
...
- [2. 우리가 실제로 측정한 것 (사실)](#2-우리가-실제로-측정한-것-사실)
  ...
  - [2.3 method 56 → 39 폐기 + 17 사용](#23-method-56--39-폐기--17-사용)
...
- [부록 B: method 17 개 + 39 폐기 list (paradigm 분류)](#부록-b-method-17-개--39-폐기-list-paradigm-분류)
...
5/14 14:00 ~ 15:21 디스코드 회의에서 임채림 박사 (채림님) 가 본 연구의 narrative 에 15가지 이상의 의문을 제기했고, 박세은 팀장은 카톡으로 4 영역 (분포 정의, 샘플 수 정합성, Neyman Q-error from→to, method runtime) 의문을 정리했다. 강재현은 single carry-over 와 multi-join 재학습의 성능 차이를 추가로 물었다.
...
기존 시스템 (pgvector / VBASE / DuckDB) 은 벡터 조건의 선택도 (selectivity) 를 33.3% / 50% / 100% 같은 고정 비율로 처리해서 잘못된 실행 계획을 만들었다. Exqutor 는 ECQO 와 Adaptive Sampling 두 갈래로 이 문제를 해결한다.
...
"skew" 는 데이터가 한쪽으로 쏠려 있는 정도를 뜻한다. 예를 들어 부품 카탈로그를 K=20 묶음으로 나눴을 때, 한 묶음에 11% 가 몰리고 다른 묶음에 3% 만 있는 경우 (DEEP 데이터셋 실제 측정값) 가 skew 가 있는 상황이다.
...
* HHI = Herfindahl-Hirschman Index. 각 그룹의 점유율 제곱의 합. K=20 이 완전 균등이면 1/20 = 0.05.
* CV = Coefficient of Variation. 그룹 크기의 표준편차 / 평균. 0 이면 모든 그룹 크기 동일.
...
| P4 차원 축소 | D 차원을 작은 차원으로 사영 | sparse_rp, pca1d, rsvd, neuram |
...
측정 과정에서 39 method 를 폐기했다. 폐기 사유는 세 카테고리로 정직히 분류한다.
...
| 정합성 위반 | 9 | halton, sobol, lhs, hammersley, dense_rp, random_projection, dbscan, ccsketch, lsh, ams_count_sketch — 큰 데이터셋에서 sample budget N=385 를 위반하거나 추정값이 외곽으로 튀는 method |

남은 17 method 가 본 연구의 사용 method 다. 전체 list 는 [부록 B](#부록-b-method-17-개--39-폐기-list-paradigm-분류) 참조.

저녁 회의 숙지 요점:
- **폐기 사유 자체가 본 연구의 contribution** — 본 논문 Exqutor 는 "분포 정보를 활용해야 한다" 까지만 언급하고 구체적인 method 카탈로그를 제공하지 않았다. 우리는 56 후보 → 39 폐기의 분류 자체를 보고서에 명시한다.
...
* z=1.96 은 95% 신뢰구간 (정규분포)
* P̂(1-P̂) 는 worst-case 분산, P̂=0.5 일 때 최대값 0.25
* e=0.05 는 margin of error 5%
...
- **Bernoulli (본 논문 default)** — 전체 데이터에서 random 385 개 추출. 그룹 정보 사용 안 함.
- **Equal allocation** — K=20 그룹 각각에 385/20 ≈ 19 개씩. 그룹 크기 무시.
- **Proportional allocation** — 각 그룹 크기 sz_j 에 비례, (sz_j/N) × 385.
- **Neyman allocation** — 그룹 크기 sz_j × 그룹 표준편차 σ_j 에 비례. 이론상 최적.
- **Anti-Neyman** — Neyman 의 반대 비례 (검증용 negative control).

다른 hyperparam 도 모두 본 논문 §V-B 그대로:
- 동적 표본 수 조정 모멘텀: m=0.9, η₀=0.1, α=50, β=1.5, γ=0.99, period=50
- HNSW (Fig5/6 측정 시): M=16, ef_construction=200, ef_search=400
- Trial: 10 회 반복, seed = trial_idx × 13 + 7, TRIM=1 (가장 큰 1개 trim)
...
- **K 변화에 둔감한 method 가 있다** — hilbert_real, hyperloglog 는 K 의 영향이 거의 없다. 이는 method 의 robustness 측면 가치.
- **SF=1 영역은 미측정** — 채림님 Q14 의문에 정직 답변. 향후 추가 측정 영역.

---

## 3. Q-error 결과 (from→to 모두 명시)

Q-error 는 "추정 카디널리티가 실제 카디널리티와 얼마나 다른지" 를 max(추정/실제, 실제/추정) 로 잰 값이다. 1.0 이 완벽한 추정, 2.0 이면 2 배 차이. 본 연구는 모든 측정의 Q-error 평균 / 중앙값을 mean / median 으로 보고한다.
...
해석 — skew 가 있는 DEEP / SIFT 에서 선택도 0.01 의 베르누이 Q-error 가 1.7 수준이다 (추정이 실제의 1.7 배 정도 빗나간다). KM20 stratified 만 적용해도 −6 ~ −9% 개선.
...
**3-way 5/10 REPORT verified (DEEP SF=100, 선택도 0.01):**
...
- Bernoulli → Proportional @ 선택도 0.01: **1.748 → 1.584 (−9.38%)** [verified]

**5-way 결과 (narrative base, CLAUDE.md):**

5 cell × 5 trial 평균 mean Q-error:
...
| Anti-Neyman | 1.540 (−11.90%, negative control 이지만 best) |

- σ_j 의 범위가 1.3 ~ 1.6 배로 좁고 그룹 크기 N_i 의 CV=0 인 환경 (모든 그룹 크기가 같다는 가정) 에서 Neyman 의 우위가 사라진다.
- Anti 가 best 인 paradox 의 원인은 §6 에서 자세히 설명.
...
채림님 회의 중 polite 한 표현으로 "왜 그 많은 method 를 폐기했나" 라고 질문. 답변은 §2.3 의 3 카테고리 분류 (자원 7 + audit 23 + 정합성 9 = 39).
...
- **Pareto frontier 가 정확도 best 와 동일**. sparse_rp / chao_weighted / reservoir 같은 method 가 정확도 + 자원 효율 모두 best.
...
> $n_j \propto N_j \cdot \sigma_j$

본 환경에서:
- $N_j$ (그룹 크기) 의 range: 3 배 (DEEP) ~ 3.13 배 (SIFT)
- $\sigma_j$ (그룹 분산) 의 range: 1.3 ~ 1.6 배
...
## 부록 B: method 17 개 + 39 폐기 list (paradigm 분류)
...
| P4 차원 축소 | ~10 | ~6 | 4 | sparse_rp / neuram / pca1d / rsvd |
...
작성: 2026-05-14 16:00 KST · 분량: 약 1400 줄 · 회의 의견 8건 + 회의 질문 15+ 가지 답변 통합 + 6 핵심 method 부록 + 환각 disclosure · 환각 0 원칙 (fact extraction agent 결과만 사용 + 검증 필요 영역 명시) · 5/14 저녁 긴급 회의 + 5/15 박광현 미팅 D-1 + 5/27 발표 + 6/11 보고서 공통 base
